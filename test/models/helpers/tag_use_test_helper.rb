module TagUseTestHelper
  ...
  def create_tag_use(account, options= {})
    ...
    test_tag_use = FactoryGirl.build(:tag_uses, 
      :tag_id => test_tag.id,
      :taggable_type => options[:taggable_type] || 'Helpdesk::Ticket', 
      :taggable_id => options[:taggable_id] || account.tickets.first.id,
      :account_id => account.id
      )
