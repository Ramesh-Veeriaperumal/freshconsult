...
class CategoriesDependencyTest < ActionDispatch::IntegrationTest
  ...
  def test_before_filters_application_controller
    expected_filters = [:determine_pod, :activate_authlogic, :clean_temp_files, :select_shard, 
      ...
      :check_account_state, :set_time_zone, :check_day_pass_usage, :check_params, :force_utf8_params, :manipulate_params, :persist_user_agent, 
      :set_cache_buster, :logging_details, :portal_check, :remove_rails_2_flash_after, :set_affiliate_cookie, :verify_authenticity_token, 
      :response_headers, :build_object, :load_object, :load_objects, :validate_params, :load_association] 
