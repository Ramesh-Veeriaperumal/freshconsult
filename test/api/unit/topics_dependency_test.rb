...
class TopicsDependencyTest < ActionView::TestCase
  def test_before_filters_web_topics_controller
    expected_filters = [:determine_pod, :activate_authlogic, :clean_temp_files, :select_shard, :unset_current_account,
      ...
      :remove_rails_2_flash_after, :set_affiliate_cookie, :check_account_activation, :verify_authenticity_token, :require_user,
      :find_topic, :portal_check, :fetch_monitorship, :set_page, :after_destroy_path, :verify_ticket_permission,
      :unset_thread_variables, :redirect_for_ticket, :set_selected_tab, :fetch_vote, :toggle_vote, :ensure_proper_sts_header]
