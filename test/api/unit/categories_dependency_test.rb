...
class CategoriesDependencyTest < ActionView::TestCase
  def test_before_filters_application_controller
    expected_filters = [:determine_pod, :activate_authlogic, :clean_temp_files, :select_shard, :unset_current_account,
      ...
      :remove_rails_2_flash_after, :set_affiliate_cookie, :verify_authenticity_token, :build_object, :load_object,
      :unset_thread_variables, :portal_check, :check_no_topics, :set_selected_tab, :content_scope, :fetch_spam_counts, :ensure_proper_sts_header, :set_ui_preference, :complete_step]
