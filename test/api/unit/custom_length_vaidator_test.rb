...
class CustomLengthValidatorTest < ActionView::TestCase
  ...
  def test_default_error_message
    ...
    assert_equal({ attribute2: { max_count: 10, current_count: 11 } }, test.error_options)

    test = TestValidation.new
    test.attribute2 = [1]
    refute test.valid?
    assert_equal(['Attribute2 test'], test.errors.full_messages)
    assert_equal({ attribute2: { max_count: 10, current_count: 1 } }, test.error_options)
  end
  ...
  def test_custom_error_message
    ...
    assert_equal({ attribute1: { max_count: 10, current_count: 11 } }, test.error_options)
  end
