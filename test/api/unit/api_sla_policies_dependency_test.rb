...
class ApiSlaPoliciesDependencyTest < ActionView::TestCase
  def test_before_filters_application_controller
    expected_filters = [:determine_pod, :activate_authlogic, :clean_temp_files, :select_shard, :unset_current_account,
      ...
      :remove_rails_2_flash_after, :set_affiliate_cookie, :verify_authenticity_token, :set_selected_tab, :load_sla_policy,
      :unset_thread_variables, :load_item, :validate_params, :initialize_escalation_level_details,
      :validate_sla_params, :ensure_proper_sts_header, :record_query_comment]
