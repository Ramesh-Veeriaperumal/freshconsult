...
class TicketFilterValidationTest < ActionView::TestCase
  ...
  def test_fsm_appointment_date_filter_with_date
    Account.stubs(:current).returns(Account.first)
    Account.current.stubs(:field_service_management_enabled?).returns(true)
    perform_fsm_operations
    query_hash_params = {
        '0' => { 'condition' => 'cf_fsm_appointment_date', 'operator' => 'is', 'value' => {from: "2018-12-02", to: "2018-12-12"}, 'type' => 'custom_field' }
      }
  ...
  def test_fsm_appointment_date_filter_with_default_dropdown_value
    Account.stubs(:current).returns(Account.first)
    Account.current.stubs(:field_service_management_enabled?).returns(true)
    perform_fsm_operations
      query_hash_params = {
        '0' => { 'condition' => 'cf_fsm_appointment_date', 'operator' => 'is', 'value' => "today", 'type' => 'custom_field' }
      }
    ticket_filter = TicketFilterValidation.new(query_hash: query_hash_params, version: 'private')
    assert ticket_filter.valid?
  ensure
  ...
  def test_fsm_appointment_date_filter_with_invalid_start_and_end_date
    Account.stubs(:current).returns(Account.first)
    Account.current.stubs(:field_service_management_enabled?).returns(true)
    perform_fsm_operations
    query_hash_params = {
      '0' => { 'condition' => 'cf_fsm_appointment_date', 'operator' => 'is', 'value' => {from: "2018-12-12", to: "2018-12-02"}, 'type' => 'custom_field' }
    }
    ticket_filter = TicketFilterValidation.new(query_hash: query_hash_params, version: 'private')
    refute ticket_filter.valid?
    error = ticket_filter.errors.full_messages
    assert error.include?('Query hash invalid_date_range')
  ensure
    cleanup_fsm
    Account.any_instance.unstub(:field_service_management_enabled?)
  end

  def test_fsm_appointment_date_filter_with_invalid_date_range
    Account.stubs(:current).returns(Account.first)
    Account.current.stubs(:field_service_management_enabled?).returns(true)
    perform_fsm_operations
    query_hash_params = {
      '0' => { 'condition' => 'cf_fsm_appointment_date', 'operator' => 'is', 'value' => {from: "2018-12-02", to: "2018-12-31"}, 'type' => 'custom_field' }
    }
    ticket_filter = TicketFilterValidation.new(query_hash: query_hash_params, version: 'private')
    refute ticket_filter.valid?
    error = ticket_filter.errors.full_messages
    assert error.include?('Query hash date_limit_exceeded')
  ensure
    cleanup_fsm
    Account.any_instance.unstub(:field_service_management_enabled?)
  end
  ...
  def test_fsm_appointment_date_filter_with_invalid_string
    ...
    perform_fsm_operations
    query_hash_params = {
      '0' => { 'condition' => 'cf_fsm_appointment_date', 'operator' => 'is', 'value' => Faker::Lorem.characters(10), 'type' => 'custom_field' }
    }
    ticket_filter = TicketFilterValidation.new(query_hash: query_hash_params, version: 'private')
    refute ticket_filter.valid?
    error = ticket_filter.errors.full_messages
    assert error.include?('Query hash query_format_invalid')
  ensure
    cleanup_fsm
    Account.any_instance.unstub(:field_service_management_enabled?)
  end  
