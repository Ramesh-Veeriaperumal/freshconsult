...
class CustomLengthValidatorTest < ActionView::TestCase
  ...
  def test_default_error_message
    ...
    assert_equal({ attribute2: { min_count: 2, max_count: 10, current_count: 11, element_type: :characters, elements: 'attribute2' } }, test.error_options)

    test = TestValidation.new
    test.attribute2 = [1]
    refute test.valid?
    assert_equal(['Attribute2 test'], test.errors.full_messages)
    assert_equal({ attribute2: { min_count: 2, max_count: 10, current_count: 1, element_type: :characters, elements: 'attribute2' } }, test.error_options)
  end
  ...
  def test_custom_error_message
    ...
    assert_equal({ attribute1: { min_count: 0, max_count: 10, current_count: 11, element_type: :characters, elements: 'attribute1' } }, test.error_options)
  end
