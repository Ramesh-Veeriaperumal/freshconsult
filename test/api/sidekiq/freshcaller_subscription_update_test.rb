...
class FreshcallerSubscriptionUpdateTest < ActionView::TestCase
  ...
  def test_omni_subscription_update_successful_update
    stub_freshcaller_omni_request(200, 'OK', { transactionId: 'S311111111', message: "success" })
    assert_nothing_raised do
      Billing::FreshcallerSubscriptionUpdate.new.perform(construct_test_subscription_params)
    end
  ensure
    unstub_freshcaller_omni_request
  end

  def stub_freshcaller_omni_request(status = 200, message = 'OK', body = {})
    Account.stubs(:current).returns(@account)
    @resp = stub_request(:post, FreshcallerSubscriptionConfig['subscription_host']).to_return(body: body.to_json,
        status: status,
        headers: { 'Content-Type' => 'application/json' })
  end

  def unstub_freshcaller_omni_request
