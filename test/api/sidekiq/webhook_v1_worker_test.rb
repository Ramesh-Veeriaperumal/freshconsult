...
class WebhookV1WorkerTest < ActionView::TestCase
  ...
  def test_webhook_failure_is_posted_to_central_worker
    ...
    #$redis_others.perform_redis_op('set', format(WEBHOOK_ERROR_NOTIFICATION, account_id: args[:account_id], rule_id: args[:rule_id]), true)
    ::WebhookV1Worker.new.perform(args)
