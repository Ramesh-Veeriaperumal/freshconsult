        ...
        FIELD_OPERATOR_MAPPING.each do |operator_type, options|
          options[:fields].each do |field_name|
            options[:operators].each do |operator|
              define_method "test_observer_condition_#{field_name}_#{operator}_#{feature}" do
                Rails.logger.debug "start test_observer_condition_#{field_name}_#{operator}_#{feature}"
