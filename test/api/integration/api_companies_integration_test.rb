...
class ApiCompaniesIntegrationTest < ActionDispatch::IntegrationTest
  def test_query_count
    ...
    # create
    v1[:create] = count_queries { post('/customers.json', company_payload, @write_headers) }
    ...
    id2 = Company.last.id
    # show
    v1[:show] = count_queries { get("/customers/#{id2}.json", nil, @headers) }
    ...
    # update
    v1[:update] = count_queries { put("/customers/#{id2}.json", company_payload, @write_headers) }
