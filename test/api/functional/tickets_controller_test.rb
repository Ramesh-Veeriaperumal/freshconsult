...
class TicketsControllerTest < ActionController::TestCase
  ...
  def test_create_with_skip_mandatory_check_without_default_fields_required_except_requester
    params = { email: Faker::Internet.email, _skip_mandatory_check: true }
    post :create, construct_params({}, params)
    t = Helpdesk::Ticket.last
    match_json(ticket_pattern(params, t))
    match_json(ticket_pattern({}, t))
  ...
  def test_create_with_skip_mandatory_check_without_default_fields_required
    params = { _skip_mandatory_check: true }
    post :create, construct_params({}, params)
    assert_response 400
    match_json([bad_request_error_pattern('requester_id', :fill_a_mandatory_field, field_names: 'requester_id, phone, email, twitter_id, facebook_id')])
  end

  def test_create_with_skip_mandatory_check_and_all_default_fields_required_invalid
    ...
    post :create, construct_params({}, subject: nil,
                                      description: nil,
                                      group_id: nil,
                                      product_id: nil,
                                      responder_id: nil,
  ...
  def test_create_with_skip_mandatory_check_without_custom_fields_required
    params = ticket_params_hash.merge(_skip_mandatory_check: true)
    Helpdesk::TicketField.where(name: [@@custom_field_names]).update_all(required: true)
    post :create, construct_params({}, params)
    Helpdesk::TicketField.where(name: [@@custom_field_names]).update_all(required: false)
    match_json(ticket_pattern(params, Helpdesk::Ticket.last))
  ...
  def test_create_with_skip_mandatory_check_with_custom_fields_required_invalid
    params = ticket_params_hash.merge(custom_fields: {}, _skip_mandatory_check: true)
    VALIDATABLE_CUSTOM_FIELDS.each do |custom_field|
      params[:custom_fields]["test_custom_#{custom_field}"] = CUSTOM_FIELDS_VALUES_INVALID[custom_field]
    end
    post :create, construct_params({}, params)
  ...
  def test_compose_email_with_skip_mandatory_check
    params = ticket_params_hash.except(:source, :product_id, :responder_id).merge(_skip_mandatory_check: true)
    post :create, construct_params({ _action: 'compose_email' }, params)
    assert_response 400
    match_json([bad_request_error_pattern('_skip_mandatory_check', :invalid_field)])
  end
