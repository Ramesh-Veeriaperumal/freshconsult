...
class ApiAgentsControllerTest < ActionController::TestCase
  ...
  def test_multiple_agent_creation_with_valid_emails_and_no_role
    ...
    shard = ShardMapping.find_by_account_id(account.id).shard_name
    ActiveRecord::Base.safe_send('switch_connection', {:shard => shard})
    assert_response 202
    pattern = { job_id: uuid, href: @account.bulk_job_url(uuid) }
    match_json(pattern)
    @account.reload
    valid_emails.each do |email|
      assert_equal Sharding.run_on_shard(shard) {@account.users.find_by_email(email).agent.present?, true}
    end
  ensure
    request.unstub(:uuid)
