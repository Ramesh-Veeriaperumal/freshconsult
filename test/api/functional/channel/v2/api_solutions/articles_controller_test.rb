...
class Channel::V2::ApiSolutions::ArticlesControllerTest < ActionController::TestCase
  ...
  def get_articles_by_portal_id(portal_id, language = Account.current.language_object)
    @account.solution_articles.joins({ solution_folder_meta: [solution_category_meta: :portal_solution_categories] }).where('portal_solution_categories.portal_id = ? AND language_id = ?', portal_id, @account.language_object.id)
  end
