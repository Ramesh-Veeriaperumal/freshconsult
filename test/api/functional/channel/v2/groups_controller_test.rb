...
class Channel::V2::GroupsControllerTest < ActionController::TestCase
  ...
  def test_sync_groups_auth_failure_403
  ...
  def test_sync_groups_source_without_permission
    job_id = SecureRandom.hex
    request.stubs(:uuid).returns(job_id)
  ...
  def test_sync_groups_success_202_respone
    set_jwt_auth_header(SOURCE)
    job_id = SecureRandom.hex
    request.stubs(:uuid).returns(job_id)
    remove_others_redis_key(resync_rate_limiter_key(SOURCE))
    remove_others_redis_key(format(CENTRAL_RESYNC_JOB_STATUS, source: SOURCE, job_id: job_id))
    expected = { 'entity_name' => 'group', 'status' => RESYNC_JOB_STATUSES[:started], 'records_processed' => '0', 'last_model_id' => '' }.freeze
  ...
  def test_sync_groups_validation_failure_400_respone_case_meta_info_missing
    set_jwt_auth_header(SOURCE)
    job_id = SecureRandom.hex
    request.stubs(:uuid).returns(job_id)
    remove_others_redis_key(resync_rate_limiter_key(SOURCE))
  ...
  def test_sync_groups_validation_failure_400_respone_case_meta_info_key_not_present
    set_jwt_auth_header(SOURCE)
    job_id = SecureRandom.hex
    request.stubs(:uuid).returns(job_id)
    remove_others_redis_key(resync_rate_limiter_key(SOURCE))
  ...
  def test_sync_groups_http_conflict_429
    set_jwt_auth_header(SOURCE)
    job_id = SecureRandom.hex
    request.stubs(:uuid).returns(job_id)
