    ...
    class DraftsControllerTest < ActionController::TestCase
      ...
      def test_save_draft_without_params
        ...
        match_json([bad_request_error_pattern('body', :datatype_mismatch, code: :missing_field, expected_data_type: :String),
                  bad_request_error_pattern('quoted_text', :datatype_mismatch, code: :missing_field, expected_data_type: :String)])
      end

      def test_save_draft_with_invalid_params
        ticket = create_ticket
        post :save_draft, construct_params({version: 'private', id: ticket.id}, { body: 'Sample text', quoted_text: 'Sample Quoted Text', cc_emails: 'ABC', bcc_emails: 'XYZ' })
        assert_response 400
