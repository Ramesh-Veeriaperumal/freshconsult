...
class FieldServiceManagementControllerTest < ActionController::TestCase
  ...
  def test_update_settings
    ...
    assert params == Account.current.account_additional_settings.additional_settings[:field_service_management]
    match_json(fetch_fsm_settings(Account.current.account_additional_settings))
    assert_response 200
    params = FSM_SETTINGS_DEFAULT_VALUES.dup
    put :update_settings, construct_params({}, params)
    assert Account.current.account_additional_settings.additional_settings[:field_service_management].empty?
    match_json(fetch_fsm_settings(Account.current.account_additional_settings))
    assert_response 200
  ensure
  ...
  def test_delete_key_from_additional_settings_hash_for_default_value
    ...
    put :update_settings, construct_params({}, params)
    assert_equal true, Account.current.account_additional_settings.additional_settings[:field_service_management].key?(UPDATE_SETTINGS_FIELDS[0].to_sym)
    params = { UPDATE_SETTINGS_FIELDS[0] => FSM_SETTINGS_DEFAULT_VALUES[UPDATE_SETTINGS_FIELDS[0]] }
    put :update_settings, construct_params({}, params)
    assert_equal false, Account.current.account_additional_settings.additional_settings[:field_service_management].key?(UPDATE_SETTINGS_FIELDS[0].to_sym)
  end
