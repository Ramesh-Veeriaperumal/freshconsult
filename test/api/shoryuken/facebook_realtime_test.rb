...
class FacebookRealtimeTest < ActionView::TestCase
  ...
  def test_convert_comments_from_admin_to_visitor_post_as_notes_invoking_delay
    ...
    comment_feed = sample_realtime_comment(@fb_page.page_id, post_id, comment_id, @fb_page.page_id, time)
    koala_comment = sample_comment_feed(post_id, @fb_page.page_id, comment_id, time)
    sqs_msg = Hashit.new(body: comment_feed.to_json)

    Koala::Facebook::API.any_instance.stubs(:get_object).returns(koala_comment['data'][0])
    Sqs::Message.any_instance.stubs(:requeue).returns(nil)
    Ryuken::FacebookRealtime.new.perform(sqs_msg)
    Koala::Facebook::API.any_instance.unstub(:get_object)
    Sqs::Message.any_instance.unstub(:requeue)

    fb_comment_id = koala_comment['data'][0]['id']

    assert_nil @account.facebook_posts.find_by_post_id(fb_comment_id)
  ensure
    Account.current.rollback(:skip_posting_to_fb)
  end

  def test_convert_comments_from_admin_to_visitor_post_as_notes_processing_post_delay
    Account.current.launch(:skip_posting_to_fb)
    user_id = rand(10**10)
    post_id = rand(10**15)
    comment_id = rand(10**15)
    ...
    comment_feed = sample_realtime_comment(@fb_page.page_id, post_id, comment_id, @fb_page.page_id, time)
    comment_feed[:data][:payload][:counter] = 1
    koala_comment = sample_comment_feed(post_id, @fb_page.page_id, comment_id, time)
    sqs_msg = Hashit.new(body: comment_feed.to_json)
