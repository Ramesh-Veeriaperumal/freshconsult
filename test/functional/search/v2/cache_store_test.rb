...
class CacheStoreTest < ActionController::TestCase
  ...
  def test_alias_key_add
    Search::V2::Store::Cache.instance.fetch(Search::V2::Store::Cache::ALIAS % { tenant_id: @tenant_id, type: SUPPORTED_TYPES.first }) do
      "#{SUPPORTED_TYPES.first}_#{@tenant_id}"
    end

    refute_nil $memcache.fetch(Search::V2::Store::Cache::ALIAS % { tenant_id: @tenant_id, type: SUPPORTED_TYPES.first })
  end

  # Test if alias key is removed
  def test_alias_key_remove
    Search::V2::Store::Cache.instance.remove(Search::V2::Store::Cache::ALIAS % { tenant_id: @tenant_id, type: SUPPORTED_TYPES.first })

    assert_nil $memcache.fetch(Search::V2::Store::Cache::ALIAS % { tenant_id: @tenant_id, type: SUPPORTED_TYPES.first })
  end
end
