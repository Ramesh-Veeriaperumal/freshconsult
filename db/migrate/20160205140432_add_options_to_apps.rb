class AddOptionsToApps < ActiveRecord::Migration
  ...
  def update_harvest
    ...
    app.options[:install] = {:require_feature => {:notice => 'You have not timesheets feature enabled', :feature_name => :timesheets}}
    app.options[:edit] = {:require_feature => {:notice => 'You have not timesheets feature enabled', :feature_name => :timesheets}}
    app.save!
  end
  ...
  def update_slack
    ...
    app.options[:install] = {:deprecated => {:notice => 'Please install new slack'}}
    app.save!
  end
end
