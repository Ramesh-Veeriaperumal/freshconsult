class UpdateSalesforceV2Config < ActiveRecord::Migration
  ...
  def up
    ...
    if salesforce_plus_app
      salesforce_plus_app.options[:oauth_url] = "/auth/salesforce_v2?origin=id%3D{{account_id}}%26falcon_enabled%3D{{falcon_enabled}}"
   ...
   def down
    salesforce_plus_app = Integrations::Application.find_by_name 'salesforce_v2'
    salesforce_plus_app.options[:oauth_url] = "/auth/salesforce_v2?origin=id%3D{{account_id}}"
