class AddTicketTemplatesToAddons < ActiveRecord::Migration
  shard :none
  ...
  def up
    ...
    execute <<-SQL
      INSERT INTO subscription_addons 
        ...
        ("Ticket Templates", 5.0, 1, "#{addon_types[:agent_quantity]}", NOW(), NOW())
    SQL
    ...
    execute <<-SQL
      INSERT INTO subscription_plan_addons 
        ...
        ((SELECT id FROM subscription_plans WHERE name = 'Sprout'), 
          (SELECT id FROM subscription_addons WHERE name = 'Ticket Templates')),
  ...
  def down    
    execute <<-SQL
      DELETE FROM subscription_plan_addons WHERE subscription_addon_id 
        IN (SELECT id FROM subscription_addons WHERE name IN ("Ticket Templates")) 
    SQL
    
    execute <<-SQL
      DELETE FROM subscription_addons WHERE name IN ("Ticket Templates") 
    SQL
  end
end