class AddProductIdToEmailConfigs < ActiveRecord::Migration
  def self.up
    ...
    add_index :email_configs, [:account_id, :product_id], :name => 'index_email_configs_on_account_id_and_product_id'
		...
		execute("update email_configs set product_id = id, primary_role =1 where primary_role = 0")
  end

  def self.down
    execute("update portals p inner join email_configs e on p.product_id = e.product_id and e.primary_role=1 set p.product_id = e.id where p.main_portal = 1")
