class UpdateOauth2Applications < ActiveRecord::Migration
  ...
  def self.up
    Sharding.execute_on_all_shards do
      ...
      if salesforce_app
        salesforce_app.options[:oauth_url] = "/auth/salesforce?origin=a_{{account_id}}"
      ...
      if nimble_app
        nimble_app.options[:oauth_url] = "/auth/nimble?origin=a_{{account_id}}"
      ...
      if google_calendar_app
        google_calendar_app.options[:oauth_url] = "/auth/google_oauth2?origin=a_pid%3D{{account_id}}%26app_name%3Dgoogle_calendar"
      ...
      if surveymonkey_app
        surveymonkey_app.options[:oauth_url] = "/auth/surveymonkey?origin=a_{{account_id}}"
      ...
      if mailchimp
        mailchimp.options[:oauth_url] = "/auth/mailchimp?origin=a_{{account_id}}"
      ...
      if constant_contact
        constant_contact.options[:oauth_url] = "/auth/constantcontact?origin=a_{{account_id}}"
