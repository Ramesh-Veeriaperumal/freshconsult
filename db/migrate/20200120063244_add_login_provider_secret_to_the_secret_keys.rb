class AddLoginProviderSecretToTheSecretKeys < ActiveRecord::Migration
  shard :all
  ...
  def self.up
      ...
      Account.find_each(batch_size: 300) do |account|
        ...
        begin
          if account_additional_settings.secret_keys.blank?
            account_additional_settings.secret_keys = { provider_login_token: SecureRandom.hex }
          else
            account_additional_settings.secret_keys[:provider_login_token] = SecureRandom.hex
          end
    ...
    Rails.logger.error "failed accounts = #{failed_accounts.inspect}" unless failed_accounts.empty?
  end
