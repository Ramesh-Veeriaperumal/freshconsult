class AddAccountIdToSolutionFolders < ActiveRecord::Migration
  def self.up
  	...
  	ActiveRecord::Base.connection.execute(%(update solution_folders sf inner join solution_categories sc on sf.category_id=sc.id set sf.account_id=sc.account_id))
  end
