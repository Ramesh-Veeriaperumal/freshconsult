class SetInBoundCountToHelpdeskTicketStates < ActiveRecord::Migration
  def self.up
    ...
    tkt_states.each do |tkt_state|
      ...
      tkt_state.assigned_at ||= tkt_state.created_at if tkt_state.tickets.responder_id
      tkt_state.first_assigned_at  ||= tkt_state.created_at if tkt_state.tickets.responder_id
      tkt_state.pending_since  ||= tkt_state.created_at if tkt_state.tickets.pending?
      tkt_state.resolved_at   ||= tkt_state.created_at if tkt_state.tickets.resolved?
