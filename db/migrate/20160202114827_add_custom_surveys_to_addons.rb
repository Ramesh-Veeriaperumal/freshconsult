class AddCustomSurveysToAddons < ActiveRecord::Migration
  shard :shard_1
  ...
  def self.up
  	...
  	execute <<-SQL
	  	INSERT INTO subscription_addons 
	  		...
	  		("Custom Surveys", 5.0, 1, "#{addon_types[:agent_quantity]}", NOW(), NOW())
	  SQL
	  ...
	  execute <<-SQL
	  	INSERT INTO subscription_plan_addons 
	  		...
	  		((SELECT id FROM subscription_plans WHERE name = "Sprout"), 
	  			(SELECT id FROM subscription_addons WHERE name = "Custom Surveys")),
  ...
  def self.down    
    execute <<-SQL
      DELETE FROM subscription_plan_addons WHERE subscription_addon_id 
        IN (SELECT id FROM subscription_addons WHERE name IN ("Custom Surveys")) 
    SQL
    
    execute <<-SQL
      DELETE FROM subscription_addons WHERE name IN ( "Custom Surveys" ) 
    SQL
  end
end
