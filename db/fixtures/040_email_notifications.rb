...
EmailNotification.seed_many(:account_id, :notification_type, [
  ...
  { :notification_type => EmailNotification::COMMENTED_BY_AGENT, 
      :account_id => account.id, :requester_notification => true, 
      :agent_notification => false,
      :requester_template => '<p>Dear {{ticket.requester.name}},<br/>There is a new comment on your Ticket (#{{ticket.id}}). You can view your ticket by visiting {{ticket.url}}<br />You can also reply to this email to add your comment.<br/>Ticket comment <br />{{comment.commenter.name}} &#8211; {{comment.body}}<br/>Sincerely,<br />{{helpdesk_name}} Support Team</p>',
