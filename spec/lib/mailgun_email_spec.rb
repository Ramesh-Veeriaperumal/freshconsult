...
RSpec.describe Helpdesk::Email::Process do
	...
	describe "Create ticket" do
  	...
  	it "non Reply_to based" do
		...
		it "with kbase in cc by requester" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :include_cc => RSpec.configuration.account.kbase_email})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			ticket_incremented?(@ticket_size)
		...
		it "by agent with kbase in cc", :focus => true do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :include_cc => RSpec.configuration.account.kbase_email, :reply => RSpec.configuration.account.agents.first.user.email})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			ticket_incremented?(@ticket_size)
		...
		it "with attachments" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :attachments => 1})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			ticket_incremented?(@ticket_size)
		...
		it "with attachments above 15 mb" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :attachments => 1, :large => 1})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			ticket_incremented?(@ticket_size)
		...
		it "with inline attachments" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :attachments => 1, :inline => 1})
		...
		it "forwarded from agent" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => RSpec.configuration.account.agents.first.user.email})
			email["body-plain"] = add_forward_content+email["body-plain"]
			email["body-html"] = add_forward_content+email["body-html"]
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			ticket_incremented?(@ticket_size)
			ticket.requester.email.should_not eql RSpec.configuration.account.primary_email_config.to_email
		end
		...
		it "from snapengage for chat" do
			...
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			ticket_incremented?(@ticket_size)
		...
		it "from blocked user" do
			...
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => user1.email})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			@account.tickets.size.should eql @ticket_size
		end
		...
		it "with unknown and actual email_config" do
			email = new_mailgun_email({:email_config => "abcde234fg@localhost.freshpo.com", :another_config => RSpec.configuration.account.primary_email_config.to_email})
			Helpdesk::Email::Process.new(email).perform
			ticket_incremented?(@ticket_size)
		end
		...
		it "with an additional TO email" do
			...
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :include_to => new_to_email})
			Helpdesk::Email::Process.new(email).perform
			ticket_incremented?(@ticket_size)
	...
	describe "Create article" do
		it "once by email" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.kbase_email, :reply => RSpec.configuration.account.agents.first.user.email})
			email[:from] = RSpec.configuration.account.agents.first.user.email
			Helpdesk::Email::Process.new(email).perform
		...
		it "article failure - non agent" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.kbase_email})
			Helpdesk::Email::Process.new(email).perform
			@account.solution_articles.size.should eql @article_size
		end

		it "with inline attachments" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.kbase_email, :reply => RSpec.configuration.account.agents.first.user.email, :attachments => 1, :inline => 1})
			email["body-html"] = email["body-html"] + "<img src=\"#{content_id}\" alt=\"Inline image 1\"><br>"
			email[:from] = RSpec.configuration.account.agents.first.user.email
			Helpdesk::Email::Process.new(email).perform
	...
	describe "Create Note" do
		...
		it "by blocked user" do
			user1 = FactoryGirl.build(:user, :account => RSpec.configuration.account,
                    :name => "samplethree", :email => "samplethree@shdjsjdsd.ccc",
                    :time_zone => "Chennai", :active => 0, :delta => 1, :language => "en")
			user1.save(validate: false)
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => user1.email})
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => user1.email})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			user1.blocked = true
			user1.save(validate: false)
		...
		it "with email_commands" do
			...
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => RSpec.configuration.account.agents.first.user.email})
			another["subject"] = another["subject"]+" [##{ticket.display_id}]"
		...
		it "by header" do
			email_id = Faker::Internet.email
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :m_id => email["Message-Id"], :reply => email_id})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			Helpdesk::Email::Process.new(another).perform
			ticket_incremented?(@ticket_size)
		...
		it "from cc" do
			email_id = Faker::Internet.email
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :include_cc => email_id})
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			another["body-html"] = another["body-html"]+" #{span_gen(ticket.display_id)} "+another["body-html"]
		...
		it "from to" do
			email_id = Faker::Internet.email
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :include_to => email_id})
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			another["body-html"] = another["body-html"]+" #{span_gen(ticket.display_id)} "+another["body-html"]
		...
		it "from same company" do
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => "sample1@#{@comp.domains}"})
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => "sample2@#{@comp.domains}"})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			another["body-html"] = another["body-html"]+" #{span_gen(ticket.display_id)} "+another["body-html"]
			Helpdesk::Email::Process.new(another).perform
		...
		it "for merged ticket" do 
			...
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			n_email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			Helpdesk::Email::Process.new(n_email).perform
			ticket2 = RSpec.configuration.account.tickets.last
		...
		it "as reply with new CC emails" do
			...
			new_to_email = Faker::Internet.email
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			another = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id, :include_cc => new_to_email})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
			another["subject"] = another["subject"]+" [##{ticket.display_id}]"
			Helpdesk::Email::Process.new(another).perform
		...
		it "as reply from a CC email" do
			...
			new_to_email = Faker::Internet.email
			email = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id})
			first_reply = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => email_id, :include_cc => new_to_email})
			second_reply = new_mailgun_email({:email_config => RSpec.configuration.account.primary_email_config.to_email, :reply => new_to_email})
			Helpdesk::Email::Process.new(email).perform
			ticket = RSpec.configuration.account.tickets.last
