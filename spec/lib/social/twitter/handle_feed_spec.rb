...
describe Social::Gnip::TwitterFeed do
  ...
  it "should create a note when a DM arrives and if dm threaded time is greater than one day" do
    account = @handle.account
    account.make_current    
  ...
  it "should create a note when a tweet is replied to" do
    ...
    reply_body.should eql(body) if GNIP_ENABLED
  end
  ...
  it "should not convert tweet with an invalid account_id in tag" do
    ...
    tweet = send_tweet_and_wait(feed)
    tweet.should be_nil
    dynamo_feed_for_tweet(@handle, feed, false, 0, @handle.id) if GNIP_ENABLED
  end

  it "should not convert tweet with an invalid twitter_handle_id in tag" do
    feed = sample_gnip_feed(@rule)
    rule = feed["gnip"]["matching_rules"].first
    rule["tag"] = "0_#{@default_stream.account_id}"
    feed["gnip"]["matching_rules"] = [rule]

    tweet = send_tweet_and_wait(feed)
    tweet.should be_nil
    dynamo_feed_for_tweet(@handle, feed, false, @handle.account_id, 0) if GNIP_ENABLED
  end
