...
module EmailHelper
	...
	attr_accessor :to, :from, :cc, :body, :reply_to, :attachments, :cid

	def new_email options={}
		set_essentials(options)
		env = get_envelope(options[:email_config], options[:another_config])
		generate_attachments(options[:attachments], options[:inline]) if options[:attachments]
		{
			:from => from,
			"To" => to,
			"Cc" => cc,
			"body-html" => Nokogiri::HTML(body).at_css('body').inner_html,
			"stripped-html" => Nokogiri::HTML(body).at_css('body').inner_html,
			"body-plain" => body,
			"stripped-text" => body,
			:headers => get_header(options[:email_config], options[:m_id]),
	...
	def set_essentials options
		...
		self.body = Faker::Lorem.paragraphs(5).join(" ")
		self.reply_to = options[:reply]
	end

	def generate_attachments number, content_id=nil
		attach = {}
		attachment_in = {}
		number.times do |i|
				file = ActionController::TestUploadedFile.new('./spec/fixtures/files/IMG_8880.jpg')#File.open('./spec/fixtures/files/IMG_8880.jpg')
	...
	def random_email
		"#{Faker::Lorem.words(1).join}@#{random_string(rand(3..10)).gsub("-","")}.#{random_domain}"
	end
	...
	def generate_emails count, email_config = nil, other_to = nil
		...
		if email_config
			...
			email_array << "#{random_name} <#{other_to}>" if other_to
		end
		email_array.shuffle!
		email_array.join(", ")
	end
	...
	def span_gen(ticket_id)
		%(<span title="fd_tkt_identifier" style='font-size:0px; min-height:0px; height:0px; opacity:0; max-height:0px; line-height:0px; color:#ffffff'>#{ticket_id}</span>)
	end

	def get_header email_config, m_id
	...
	def con_ids attach
		...
		attach["content-id-mapping"] = {cid => "attachment1"}.to_json
		attach
	end

	def attachment_info_cid attachment_in
		attachment_in["attachment-1"].merge!({"content_id" => cid})
	end
