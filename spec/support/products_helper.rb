module ProductsHelper
	...
	def create_product(option={})
		...
		test_email_config = FactoryGirl.build(:email_config, :to_email => option[:email], :reply_email => option[:email],
			                                             ...
			                                             :account_id => RSpec.configuration.account.id,:active=>"true")
		test_email_config.save(validate: false)

		if option[:portal_url]
			test_portal = FactoryGirl.build(:portal, :name=> option[:portal_name] || Faker::Name.name, :portal_url => option[:portal_url], :language=>"en",
				                                 :product_id => test_product.id, :forum_category_id=>"", :solution_category_ids=>[""],:account_id => RSpec.configuration.account.id,
