module Wf::FilterHelper
  ...
  def prep_ticket
    ...
    3.times do # populating objects
      @company = RSpec.configuration.account.customers.create(Factory.attributes_for(:company))
      @group = RSpec.configuration.account.groups.create(Factory.attributes_for(:group))
      ...
      @tag = RSpec.configuration.account.tags.create(Factory.attributes_for(:tag))
      @test_agent = add_test_agent(@account)
      @ticket = RSpec.configuration.account.tickets.create(Factory.attributes_for(:ticket, :requester_id => @requester.id, :responder_id => @test_agent.id, :group_id => @group.id, :product_id => @product.id, :created_at => 4.days.from_now))
      @ticket.tags = [@tag]
