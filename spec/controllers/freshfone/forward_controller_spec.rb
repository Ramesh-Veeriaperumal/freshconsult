...
RSpec.describe Freshfone::ForwardController do
  ...
  it 'should connect the direct dial and should update call metrics attribute (answered_at)' do
    ...
    create_freshfone_call
    Freshfone::Providers::Twilio.any_instance.stubs(:redirect_call)
    params = { :CallSid => @freshfone_call.call_sid,
      CallStatus: 'in-progress' }
    set_twilio_signature("freshfone/forward/direct_dial_accept", params)
    post :direct_dial_accept, params
    ...
    Freshfone::Providers::Twilio.any_instance.unstub(:redirect_call)
  end
  ...
  it 'should update hangup_at in call_metrics when direct dial call completed' do
    ...
    @freshfone_call.reload
    params = conference_call_params.merge!({ CallStatus: 'completed' })
    set_twilio_signature('freshfone/forward/direct_dial_complete', params)
    post :direct_dial_complete, params
    call_metrics = @freshfone_call.call_metrics.reload
  ...
  it 'should update ivr time in call metrics after accepting in direct dial' do
    @account.features.freshfone_call_metrics.create
    Freshfone::Providers::Twilio.any_instance.stubs(:redirect_call)
    create_freshfone_call
    Freshfone::Notifier.any_instance.stubs(:ivr_direct_dial).raises(StandardError, 'This is an exceptional message')
    params = { :CallSid => @freshfone_call.call_sid, :ConferenceSid => 'ConSid', :Timestamp => "#{Time.now.utc}" }
    set_twilio_signature("freshfone/forward/direct_dial_accept", params)
    post :direct_dial_accept, params
    call_metrics = @freshfone_call.call_metrics.reload
  ...
  it 'should update the ringing_at in metrics while waiting for transfer' do
    ...
    params = incoming_params.merge(
      'ConferenceSid' => 'ConSid', 'CallStatus' => 'ringing',
      :agent => @agent.id,
      'Timestamp' => "#{Time.now.utc}")
    set_twilio_signature("freshfone/forward/transfer_wait", params.except(:agent))
    post :transfer_wait, params
    @freshfone_call.reload
    expect(@freshfone_call.call_metrics.ringing_at).not_to be_nil
    @account.features.freshfone_call_metrics.destroy
  end
