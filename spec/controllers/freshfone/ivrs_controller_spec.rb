...
describe Freshfone::IvrsController do
  ...
  it 'should not update the welcome message on preview' do
    ...
    ivr = @number.ivr
    put :update, params
    RSpec.configuration.account.ivrs.find(ivr.id).menus.first.message.should_not be_eql(message)
  end
  ...
  it 'should enable ivr on successful activate action' do
    ...
    RSpec.configuration.account.ivrs.find(ivr.id).should be_active
  end
  ...
  it 'should destroy ivr on destroy action' do
    ...
    number = RSpec.configuration.account.freshfone_numbers.create!( :number => Faker::PhoneNumber.phone_number, 
      :display_number => "+1234567890", :country => "US", :region => "Texas", :number_type => 1 )
    post :destroy, {:id => number.ivr.id}
    RSpec.configuration.account.freshfone_numbers.find(number.id).ivr.should be_nil
  end
  ...
  it 'should not destroy ivr on destroy action' do
    ...
    number = RSpec.configuration.account.freshfone_numbers.create!( :number => Faker::PhoneNumber.phone_number, 
      :display_number => "+1234567890", :country => "US", :region => "Texas", :number_type => 1 )
    post :destroy, {:id => number.ivr.id}
    RSpec.configuration.account.freshfone_numbers.find(number.id).ivr.should_not be_nil
  end
