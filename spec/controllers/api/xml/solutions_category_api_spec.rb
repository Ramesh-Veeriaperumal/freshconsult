...
RSpec.describe Solution::CategoriesController do
  ...
  it "should be able to view a solution category" do
    ...
    result = parse_xml(response)
    expect(response.status).to be_eql(200)
    expect(assert_array(result["solution_category"].keys, APIHelper::SOLUTION_CATEGORY_ATTRIBS, ["folders"])).to be_empty
  end
  ...
  it "should be able to view all solution categories" do
    ...
    result = parse_xml(response)
    expect(response.status).to be_eql(200)
    expect(assert_array(result["solution_categories"].first.keys, APIHelper::SOLUTION_CATEGORY_ATTRIBS, ["folders"])).to be_empty
  end
