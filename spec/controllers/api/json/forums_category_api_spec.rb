...
describe ForumCategoriesController do
  ...
  it "should be able to view all forum categories" do
    ...
    result = parse_json(response)
    expected = (response.status === "200 OK")  && (compare(result.first["forum_category"].keys,APIHelper::FORUM_CATEGORY_ATTRIBS,{}).empty?)
    expected.should be(true)
  end
  it "should be able to delete a forum category" do
    ...
    result = parse_json(response)
    expected = (response.status === "200 OK") && (compare(result["forum_category"].keys,APIHelper::FORUM_CATEGORY_ATTRIBS,{}).empty?)
    expected.should be(true)
  end
