...
describe ::ForumsController do
  ...
  it "should be able to update a forum" do
    ...
    put :update, { :id => test_forum.id, :category_id => @category.id, :forum => {:description => Faker::Lorem.paragraph }, :format => 'json'}
    response.status.should === 200
  end
  it "should be able to view a forum" do
    test_forum = create_test_forum(@category)
    get :show, { :id => test_forum.id,  :category_id => @category.id,:format => 'json'}
    result = parse_json(response)
  ...
  it "should be able to delete a forum " do
    test_forum = create_test_forum(@category)
    delete :destroy, { :id => test_forum.id,  :category_id => @category.id, :format => 'json'}
    response.status.should === 200
  end
