...
describe Solution::CategoriesController do
  ...
  before(:all) do
    ...
    @test_category2 = create_category( {:name => "test category #{Faker::Name.name}", :description => "#{Faker::Lorem.sentence(3)}", :is_default => false} )
    @test_category3 = create_category( {:name => "test category #{Faker::Name.name}", :description => "#{Faker::Lorem.sentence(3)}", :is_default => false} )
    @test_default_category = create_category( {:name => "test default category #{Faker::Name.name}", :description => "#{Faker::Lorem.sentence(3)}", :is_default => true} )
  end
