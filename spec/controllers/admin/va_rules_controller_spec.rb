...
describe Admin::VaRulesController do
  ...
  before(:all) do
    @test_va_rule=create_dispatchr_rule({:account_id=>RSpec.configuration.account.id})
  end
  ...
  it "should create a new dispatchr rule" do
    ...
    RSpec.configuration.account.all_va_rules.find_by_name(va_rule_name).should_not be_nil
  end
  ...
  it "should update a dispatchr rule" do
    ...
    RSpec.configuration.account.all_va_rules.find_by_id(@test_va_rule.id).action_data.should_not be_eql(@test_va_rule.action_data)
  end

  it "should delete a dispatchr rule" do
    delete_va_rule_id = create_dispatchr_rule({:account_id => RSpec.configuration.account.id}).id
    delete :destroy, {:id=>delete_va_rule_id}
    RSpec.configuration.account.all_va_rules.find_by_id(delete_va_rule_id).should be_nil
  end

  it "should deactivate and activate a dispatchr rule" do
    va_rule = create_dispatchr_rule({:account_id => RSpec.configuration.account.id})
    put :activate_deactivate,  {:va_rule=>{:active=>"false"},:id =>va_rule.id}
    RSpec.configuration.account.all_va_rules.find_by_id(va_rule.id).active.should be_eql(false)
    put :activate_deactivate,  {:va_rule=>{:active=>"true"},:id =>va_rule.id}
    RSpec.configuration.account.all_va_rules.find_by_id(va_rule.id).active.should be_eql(true)
  end

  it "should cascade dispatchr rules" do
    RSpec.configuration.account.features.send(:cascade_dispatchr).create
    post :toggle_cascade, {:cascade_dispatcher=>"0","_"=>""}
    RSpec.configuration.account.reload
    RSpec.configuration.account.features?(:cascade_dispatchr).should be_falsey
  end
