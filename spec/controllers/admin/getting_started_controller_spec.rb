...
describe Admin::GettingStartedController do
	...
	it "should rebrand the account" do
		put :rebrand, {
			...
			:main_portal_attributes=>{ :name=>"Test Account", 
				                       :logo_attributes=>{:content=>Rack::Test::UploadedFile.new('spec/fixtures/files/image4kb.png', 'image/png')}, 
	...
	it "should not allow invalid colors while rebranding" do
		put :rebrand, {
			:account=>{ 
			:main_portal_attributes=>{ :name=>"Test Account", 
				                       :logo_attributes=>{:content=>Rack::Test::UploadedFile.new('spec/fixtures/files/image33kb.jpg', 'image/jpg')}, 
