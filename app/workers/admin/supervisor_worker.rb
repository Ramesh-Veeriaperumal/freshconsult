  ...
  class SupervisorWorker < BaseWorker
    ...
    def perform
      execute_on_db {
        ...
        Timeout::timeout(SUPERVISOR_TIMEOUT) {
          supervisor_rules.each do |rule|
            begin
              ...
              tickets_count = 0
              ticket_ids = []
              account.tickets.where(negate_conditions).where(conditions)
              .updated_in(1.month.ago).visible.joins(joins).find_each do |ticket|
                tickets_count +=  1
        ...
        Va::Logger::Automation.log "Timeout Error in Supervisor Worker - #{timeouterr}  - current_account - #{Account.current.id}"
      ensure
