...
# obj_id, user_email (for user_id) & text (data)
...
class NERWorker < BaseWorker
  ...
  def encrypt_pii(key)
    encrypted_data = Encryptor.encrypt(value: key, key: NER_API_TOKENS['secret_key'], iv: NER_API_TOKENS['iv'])
  end
end