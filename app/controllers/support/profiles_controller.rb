class Support::ProfilesController < SupportController
  ...
  def update
    if @profile.update_attributes(params[:user])
      ...
      redirect_to edit_support_profile_path(current_portal.multilingual? ? { :url_locale => Language.for_current_user.code } : {} ) # should check with parsu for all redirect_to :back
    else
