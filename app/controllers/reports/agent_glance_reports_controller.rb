class Reports::AgentGlanceReportsController < ApplicationController
  ...
  def saved_reports
    @report_filter_data = report_filter_data_hash REPORT_TYPE_BY_KEY[:agent_glance]
    @report_type = REPORT_TYPE_BY_KEY[:agent_glance]
  end

  def generate
    glance_report_data
    @report_title = t('adv_reports.agent_at_a_glance')
  ...
  protected
  def agent_name
    unless params[:agent_select_field].blank? 
      action_hash = params[:agent_select_field]
