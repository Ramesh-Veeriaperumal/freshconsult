class Fdadmin::FreshfoneStats::RenewalController < Fdadmin::DevopsMainController
  ...
  around_filter :select_slave_shard , :only => [:renewal_backlog_csv_by_account,:failed_renewal_csv_by_account]

  def renewal_backlog_csv
    params.merge!(:export_type => "Renewal Backlog")
    backlog_list = cumulative_result(Sharding.run_on_all_slaves {ActiveRecord::Base.connection.execute(all_renewal_backlog)})
  ...
  def failed_renewal_csv
    params.merge!(:export_type => "Failed Renewal")
    failed_renewal_list = cumulative_result(Sharding.run_on_all_slaves {ActiveRecord::Base.connection.execute(all_failed_renewal)})
  ...
  private
    ...
    def all_renewal_backlog
      "SELECT accounts.id, accounts.name, SUM(freshfone_other_charges.debit_payment)
      FROM freshfone_accounts JOIN accounts ON freshfone_accounts.account_id = accounts.id 
      AND freshfone_accounts.state =  #{Freshfone::Account::STATE_HASH[:suspended]}
      JOIN freshfone_other_charges ON freshfone_other_charges.account_id = accounts.id 
      WHERE freshfone_accounts.expires_on > UTC_TIMESTAMP()
      AND freshfone_other_charges.action_type = #{Freshfone::OtherCharge::ACTION_TYPE_HASH[:number_renew]}
    ...
    def failed_renewal_by_account_csv_columns
      ["Account ID", "Account Name", "Freshfone Number ID", "Freshfone Number", "Last Renewal Date"]
    end
