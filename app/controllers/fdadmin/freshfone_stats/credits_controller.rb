class Fdadmin::FreshfoneStats::CreditsController < Fdadmin::DevopsMainController
  ...
  private
    ...
    def freshfone_credits_in_date_range(start_date,end_date,status)
      "SELECT accounts.id, accounts.name, freshfone_payments.purchased_credit AS credits, 
      freshfone_payments.status, freshfone_payments.created_at FROM freshfone_payments JOIN accounts
    ...
    def all_accounts_results(startDate,endDate)
      ...
      case params[:export_type]
        ...
        when "promotional-credits"
          ...
          promotional_credits_list = get_all_credits_list(Sharding.run_on_all_slaves { ActiveRecord::Base.connection.execute(promotional_credits_query) })
        ...
        when "refunded-credits"
          ...
          refunded_credits_list = get_all_credits_list(Sharding.run_on_all_slaves { ActiveRecord::Base.connection.execute(refunded_credits_query) })
    ...
    def single_account_results(startDate, endDate)
      ...
      promotional_credits_conditions = ['created_at >= ? and created_at <= ? and status_message = ?', startDate, endDate, 'promotional']
      refunded_credits_conditions = ['created_at >= ? and created_at <= ? and status_message = ?', startDate, endDate, 'refunded']
      case params[:export_type]
        when "promotional-credits"
