  ...
  module FreshfoneStats
    class PaymentsController < Fdadmin::DevopsMainController
      ...
      def statistics
        ...
        stats = { payments: @other_credits, promotional: @promotional_credits,
                  refunded: @refunded_credits }
        stats[:signups] = cumulative_result(
          Sharding.run_on_all_slaves do
            ActiveRecord::Base.connection.execute(freshfone_signups)
          end
        )
      ...
      def freshfone_signups
        ...
        JOIN subscriptions ON subscriptions.account_id = accounts.id WHERE
        freshfone_accounts.created_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 DAY)
