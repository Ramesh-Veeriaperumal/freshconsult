...
class SubscriptionsController < ApplicationController
  ...
  private
    ...
    def perform_ui_based_addon_operations
      ...
      [SAAS::SubscriptionEventActions::ADD, SAAS::SubscriptionEventActions::DROP].each do |action|
        feature_list = if action == SAAS::SubscriptionEventActions::ADD
                         features_to_add(modified_addon_params)
                       else
                         features_to_remove(modified_addon_params)
                       end
    ...
    def features_to_add(modified_addon_params)
      features = modified_addon_params.map do |feature, value|
        feature_name = feature.to_sym
        features_list(feature_name) if !scoper.account.has_feature?(feature_name) && value['enabled'] == 'true'
      end
      features.flatten.compact
    end

    def features_to_remove(modified_addon_params)
      features = modified_addon_params.map do |feature, value|
        feature_name = feature.to_sym
        features_list(feature_name) if scoper.account.has_feature?(feature_name) && value['enabled'] == 'false'
      end
      features.flatten.compact
    end

    def features_list(feature_name)
      dependent_features = SubscriptionConstants::USER_CONFIGURABLE_ADDON_DEPENDENT_FEATURES_MAP[feature_name]
      dependent_features.presence || [feature_name]
    end
