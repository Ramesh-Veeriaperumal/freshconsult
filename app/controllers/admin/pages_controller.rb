class Admin::PagesController < Admin::AdminController
  ...
  before_filter(:only => [:update]) do |c|
    c.safe_send(:liquid_syntax?, c.request.params[:portal_page][:content])
  end
  ...
  private
    ...
    def get_redirect_portal_url
      ...
      portal_redirect_url = safe_send(method_name)
      begin
        ...
        unless cname.blank?
          ...
          portal_redirect_url = safe_send(method_name, :id => id) 
        else
          portal_redirect_url = safe_send(method_name)
        end
      rescue Exception => e
