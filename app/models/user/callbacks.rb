...
# of admin to agent or agent to admin check
  ...
  def send_alert_email
    if agent?
      ...
      SecurityEmailNotification.send_later(:deliver_agent_email_change, self, self.email_was,
        changed_attributes_names, User.current, false, { :locale_object => self })
      if User.current.email.present? && User.current.email != self.email_was
          SecurityEmailNotification.send_later(:deliver_agent_email_change, self, User.current.email,
          changed_attributes_names, User.current, { :locale_object => User.current })
      end
    end
  end
