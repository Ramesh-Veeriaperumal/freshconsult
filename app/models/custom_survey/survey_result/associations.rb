class CustomSurvey::SurveyResult < ActiveRecord::Base
  ...
  scope :remarks , lambda { |condition| { 
      :include => [:survey_remark, :customer, :agent, :group],
  ...
  scope :rating_filter, lambda { |condition| { 
      :include => :survey,
      :joins => :survey_result_data,
      :conditions => ["survey_result_data.#{condition[:column_name]}=#{condition[:value]}"]
  }}
  ...
  scope :report, lambda { |condition| {
      :select => "`survey_results`.id, `survey_results`.survey_id, survey_result_data.#{condition[:column_name]} as rating, count(*) as total",
      :joins => "INNER JOIN `survey_result_data` ON `survey_result_data`.`survey_result_id` = `survey_results`.`id` AND `survey_result_data`.`account_id` = `survey_results`.`account_id`",
  ...
  scope :group_wise, lambda { |condition| {
      :select => "group_id as id, groups.name as name, survey_result_data.#{condition[:column_name]} as rating, count(*) as total",
      :joins => [:survey_result_data,:group],
  ...
  scope :agent_wise, lambda {|condition| {
      :select => "agent_id as id, users.name as name, survey_result_data.#{condition[:column_name]} as rating, count(*) as total",
      :joins => [:survey_result_data,:agent],
