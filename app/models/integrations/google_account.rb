...
class Integrations::GoogleAccount < ActiveRecord::Base
  ...
  attr_encrypted :token, random_iv: true, compress: true
  attr_encrypted :secret, random_iv: true, compress: true
  validates :encrypted_token, symmetric_encryption: true
...
#      puts "Complete goog_contact_detail "+goog_contact_detail.inspect + ", sync_tag_id #{sync_tag_id}, user detail "+user.inspect
      ...
      goog_contact_detail.each { |key, value|
        ...
        user.safe_send(:write_attribute,db_attr, value) unless (db_attr.blank? or value.blank?)
      }
