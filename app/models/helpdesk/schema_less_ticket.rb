class Helpdesk::SchemaLessTicket < ActiveRecord::Base
  ...
  ['agent', 'group', 'internal_agent', 'internal_group'].each do |type|
    ...
    def update_lifecycle_changes(event_time, group, resolved_or_closed)
        ...
        action_time_hash = {
            action_time_in_bhrs: action_time_in_bhrs < 1 ? 0 : action_time_in_bhrs,
  ...
  def channel_message_id
    additional_info[:channel_message_id]
  end

  def channel_message_id=(value)
    additional_info[:channel_message_id] = value
  end
