class Helpdesk::SchemaLessTicket < ActiveRecord::Base
  ...
  after_commit :backup_change # This neeeds to be at last
	...
	def set_first_response_id(note_id)
      ...
      Rails.logger.info "Helpdesk::SchemaLessTicket::set_first_response_id::#{Time.zone.now.to_f} and schema_less_ticket_object :: #{self.reports_hash.inspect}"
      self.save
		end
	end
  ...
  def update_first_response_agent_id
    ...
    Rails.logger.info "Helpdesk::SchemaLessTicket::update_first_response_agent_id::#{Time.zone.now.to_f} and schema_less_ticket_object :: #{self.reports_hash.inspect}"
    self.save
