class Helpdesk::SchemaLessNote < ActiveRecord::Base
	...
	def cc_and_bcc_emails_count
		if ((!cc_emails.blank? && cc_emails.count >= TicketConstants::MAX_EMAIL_COUNT) || 
				(!bcc_emails.blank? && bcc_emails.count >= TicketConstants::MAX_EMAIL_COUNT) ||
				(!to_emails.blank? && to_emails.count >= TicketConstants::MAX_EMAIL_COUNT))
