  ...
  def deliver_password_reset_instructions!(portal) #Do we need delayed_jobs here?! by Shan
    ...
    UserNotifier.deliver_password_reset_instructions(self, 
        :email_body => Liquid::Template.parse(template).render((user_key ||= 'agent') => self, 
          'helpdesk_name' => (!portal.name.blank?) ? portal.name : account.portal_name , 'password_reset_url' => edit_password_reset_url(perishable_token, :host => (!portal.portal_url.blank?) ? portal.portal_url : account.host)) , :reply_email => portal.product.friendly_email)
  end
