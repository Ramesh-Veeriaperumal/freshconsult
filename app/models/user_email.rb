...
class UserEmail < ActiveRecord::Base
  ...
  before_save :construct_model_changes, if: -> { !self.primary_role }
  ...
  before_destroy :save_changes_before_destroy, if: -> { !self.primary_role }
  ...
  private
    ...
    def change_email_status
      self.verified = false unless user.freshid_enabled_and_agent?
      true
    end
    ...
    def freshid_integration_enabled_account?
      Account.current.freshid_integration_enabled?
    end
end
