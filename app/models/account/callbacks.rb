  ...
  private
    ...
    def create_shard_mapping
      if Fdadmin::APICalls.non_global_pods? && domain_mapping = DomainMapping.find_by_domain(full_domain) 
        ...
        shard_mapping = ShardMapping.new({:shard_name => ShardMapping.current_shard_selection.shard.nil? ? ShardMapping.latest_shard : ShardMapping.current_shard_selection.shard.to_s,:status => ShardMapping::STATUS_CODE[:not_found],
    ...
    def domain_already_exists?
      ...
      domain_mapping.present? && self.id != domain_mapping.account_id
    end
end
