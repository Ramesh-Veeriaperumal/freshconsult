module Helpdesk::IssuesHelper

  DEFAULT_FILTER = [:open, :visible]

  CONTEXTS = [:all, :open]

  SELECTORS = [
    [[:unassigned],         "New Issues"                     ],
    [[:responded_by],       "My Issues"                      ],
    [[:visible],            "All Issues"                     ],
    [[:deleted],            "Trash",                  [:all] ]
  ]
  ...
  def filter_link(selector)
    if selector == current_selector
     SELECTOR_NAMES[selector]
      ...
      link_to(SELECTOR_NAMES[selector], helpdesk_filter_issues_url(filter(selector)))
    end
  end
  ...
  def filter_count(selector=nil)
     Helpdesk::Issue.filter(filter(selector), current_user).count
  end
  ...
  def search_fields
    select_tag(
      :f, 
      options_for_select(Helpdesk::Issue::SEARCH_FIELD_OPTIONS, (params[:f] && params[:f].to_sym)), 
