...
HTML
	...
	def construct_ticket_element(object_name, field, field_label, dom_type, required, field_value = "", field_name = "", in_portal = false , is_edit = false)
	    ...
	    if field.editable_in_portal && field.visible_in_view_form?
	    	label = label_tag object_name+"_"+field.field_name, field_label, :class => ((dom_type != "checkbox") ? 
	    		("#{required ? 'required': ""} control-label") : "")
	    else
	    	label = content_tag :h4, field_label
	    end
	    ...
	    case dom_type
	      ...
	      when "nested_field" then
	        element = label + content_tag(:div, nested_field_tag(object_name, field_name, field, {:include_blank => "...", :selected => field_value}, {:class => element_class + " custom-select"}, field_value, in_portal), :class => "controls")
	      when "hidden" then
