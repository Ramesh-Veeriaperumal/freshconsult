module EmailMailboxConstants
  ...
  CUSTOM_MAILBOX_HASH_FIELDS = [custom_mailbox: ['access_type', 'reference_key'].freeze | INCOMING_HASH_FIELDS | OUTGOING_HASH_FIELDS].freeze
  CREATE_FIELDS = %w[name support_email default_reply_email group_id product_id mailbox_type].freeze | CUSTOM_MAILBOX_HASH_FIELDS

  INDEX_FIELDS = %w[order_by order_type product_id group_id support_email forward_email active].freeze

  UPDATE_FIELDS = CREATE_FIELDS
  VALIDATABLE_DELEGATOR_ATTRIBUTES = %w[imap_mailbox_attributes smtp_mailbox_attributes primary_role group_id product_id].freeze
