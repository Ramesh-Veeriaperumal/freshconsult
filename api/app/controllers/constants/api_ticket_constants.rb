module ApiTicketConstants
  ...
  CREATE_FIELDS = %w(description due_by email_config_id fr_due_by group_id 
                    internal_group_id priority email phone twitter_id facebook_id 
                    requester_id name internal_agent_id responder_id source 
                    status subject type product_id company_id unique_external_id
                  ).freeze | ARRAY_FIELDS | HASH_FIELDS
  UPDATE_FIELDS = %w(description due_by email_config_id fr_due_by group_id 
                    internal_group_id priority email phone twitter_id facebook_id 
                    requester_id name internal_agent_id responder_id source 
                    status subject type product_id company_id unique_external_id
