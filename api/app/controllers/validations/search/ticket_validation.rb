module Search
	class TicketValidation < ApiValidation
	  ...
	  validates :status, data_type: { rules: Array }, array: { custom_inclusion: { in: Account.current.ticket_status_values_from_cache.map(&:status_id), ignore_string: :allow_string_param, detect_type: true } }
    validates :priority, data_type: { rules: Array }, array: { custom_inclusion: { in: ApiTicketConstants::PRIORITIES, ignore_string: :allow_string_param, detect_type: true } }
