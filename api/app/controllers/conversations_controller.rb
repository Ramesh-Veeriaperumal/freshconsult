class ConversationsController < ApiApplicationController
  ...
  private
    ...
    def fb_public_api?
      public_api? && Account.current.launched?(:facebook_public_api) && (@ticket[:source] == Helpdesk::Source::FACEBOOK) && reply?
    end

    def twitter_public_api?
      public_api? && Account.current.launched?(:twitter_public_api) && (@ticket[:source] == Helpdesk::Source::TWITTER) && reply?
    end
