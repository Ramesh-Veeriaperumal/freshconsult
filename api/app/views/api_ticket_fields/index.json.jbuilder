json.array! @items do |tf|
  json.cache! CacheLib.key(tf, params) do
    json.extract! tf, :id, :default, :description, :label, :name, :position, :required_for_closure, :created_at, :updated_at
    ...
    json.set! :nested_ticket_fields do
      json.array! tf.nested_ticket_fields do |tf_nested_field|
        json.cache! CacheLib.key(tf_nested_field, params) do
          json.extract! tf_nested_field, :description, :id, :label, :label_in_portal, :level, :name, :ticket_field_id, :created_at, :updated_at
        end
      end
    end
  end
