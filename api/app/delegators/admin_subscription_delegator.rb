class AdminSubscriptionDelegator < BaseDelegator
  ...
  def initialize(item, options = {})
    ...
    @renewal_period = options[:renewal_period]
    @subscription = Account.current.subscription
    super(item, options)
  end
  
  def validate_not_active_plan
    errors[:state] << :not_active_account if !@subscription.active? || @subscription.card_number.blank?
  end
