    ...
    def validate_contact
      @contact_conditions.each do |contact|
        if CONDITION_CONTACT_FIELDS.include?(contact[:field_name].to_sym)
          ...
          field_not_found_error("contact[#{contact[:field_name]}]") if custom_field.blank?
          ...
          validate_case_sensitive(contact, custom_field.dom_type) if contact.has_key? :case_sensitive
