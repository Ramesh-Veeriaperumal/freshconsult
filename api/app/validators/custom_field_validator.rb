class CustomFieldValidator < ActiveModel::EachValidator
  ...
  private
    ...
    def required_field?(record, values)
      ...
      is_required = @current_field.safe_send(@required_attribute.to_sym) || (@closure_status && @current_field.required_for_closure)
    ...
    def parent_value(parent_field, record, values)
      record.respond_to?(parent_field) ? 
        (record.safe_send(parent_field) rescue values.try(:[], parent_field)) : 
        values.try(:[], parent_field)
    end
