        ...
        return{
          ...
          window.liveChat.request('chats/' + chatId + '/getUnmarkedMsgs', 'GET', requestData, function(err, data){
            ...
            createTicketHelpkit: function(data){
                // console.log("ENTER createTicketHelpkit: ", JSON.stringify(data));
                if(!data){
                ...
                $.ajax({
                    ...
                    data: {"ticket":ticket},
                    success: function(response) {
                        //console.log("INFO main.js - AJAX /livechat/create_ticket - SUCCESS response:",
                        //    JSON.stringify(response));
                        if (response.status === true) {
                            // console.log("INFO livechat/ticketOptions/main.js AJAX - response.status === true");
                            ...
                            // the create ticket api call will return a 200
                            // although it has failed - this is esp for domain restriction
                            // {"status":"error","message":"User email does not belong to a supported domain."}
                            // console.log("ERROR creating tkt - AJAX response.status !== true invoking flashNotice with message:",
                            //             response.message);
                            that.flashNotice("ticket", false, null, response.message);
                            that.closeWindow(null, null, true);
                        }
                    },
                    error: function (errmsg) {
                        // console.log("ERROR main.js - AJAX /livechat/create_ticket  createTicketHelpkit: errmsg ", errmsg);
            ...
            flashNotice: function(type, status, ticket_id, detailed_error_message) {
                // console.log("main.js flashNotice /livechat/ticketOptions",
                //            ", type:", type, ", status: ", status, 
                //            "ticket_id:", ticket_id,
                //            ", detailed_error_message:", detailed_error_message);
                var msg;
                var RESTRICTED_DOMAIN_FAILURE_RESPONSE = "User email does not belong to a supported domain."; 
                if (type === "ticket") {
                    if (status === true) {
                        msg = CHAT_I18n.ticket_success_msg+"<a data-pjax='#body-container' href='/helpdesk/tickets/"+
                            ticket_id+"'>"+" "+CHAT_I18n.view_details+"</a>";
                    } else {
                        // we are in an error state right now - need to
                        // report the correct error 
                        // we have a specific error for domain restriction
                        // otherwise create a generic error
                        if (detailed_error_message &&
                            detailed_error_message === RESTRICTED_DOMAIN_FAILURE_RESPONSE) {
                            // console.log("INFO - RESTRICTED_DOMAIN_FAILURE_RESPONSE livechat/ticketOptions/main.js , detailed_error_message:",
                            //             detailed_error_message);
                            msg = CHAT_I18n.ticket_domain_restricted_error_msg;
                        } else {
                            // console.log("INFO - ELSE livechat/ticketOptions/main.js , detailed_error_message:",
                            //             detailed_error_message);
                            msg = CHAT_I18n.ticket_error_msg;
                        }
                    }
                } else if (type === "note") {
                    if (status === true) {
                        msg = CHAT_I18n.note_success+" #"+ticket_id+".<a data-pjax='#body-container' href='/helpdesk/tickets/" +
                            ticket_id+"'>"+" "+CHAT_I18n.view_details+"</a>";
                    } else {
                        msg = CHAT_I18n.note_error;
                    }
                }
                // console.log("INFO livechat/ticketOptions/main.js, flashNotice msg :", msg);
                if (msg != "") {
