        ...
        data:function(){
           ...
           var q_length = survey.survey_questions.length;
            for(var i=0;i<q_length;i++){
                ...
                surveyQuestion = survey.survey_questions[i].label;
                tab.label = trim(surveyQuestion.substring(surveyQuestion.indexOf(":") + 1));
                tabs.push(tab);
            }
        ...
        renderSidebar:function(){
            /*    if(SurveyTab.isRequired()){  */
                    jQuery("#survey_report_sidebar").html(
                        JST["survey/analytics/template/stats_tab_layout"]()
                    );
      ...
      /*          }
        ...
        ToolTip:{
            showToolTip:function(object,id,type,title){
                ...
                data.empty();
                data.html(JST["survey/analytics/template/content_summary"]({
              ...
              /*  data.find('.muted').remove();
                data.find('.overview-percentage').removeClass('overview-percentage'); */
                jQuery(object).qtip({
                  style: {
