 ...
 window.faye_subscriptions = [];
  var CLIENT_DEFAULT_OPTIONS = {
    ...
    bindUnloadEvnts = function(){
      ...
      jQuery(document).bind('disconnectNode', function(ev){
        window.fayeClient.disconnect();
      });
    };

  FreshdeskNode.prototype.addAuthExt = function(params){
    window.fayeClient.addExtension({
  ...
  var currentUserId,
    ...
    refreshCallBack = function(message){
        console.log('I have a callback for refreshCallBack ');
    ...
    var Freshdesk_Collision_Node_Methods = {
      init : function(host,opts)
      {
        ...
        if(!window.fayeClient)
        {
          window.fayeClient = new Faye.Client(this.host,this.opts);
        }
        this.client  = window.fayeClient;
      },
