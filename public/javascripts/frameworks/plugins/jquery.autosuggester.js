    ...
    Autocomplete.prototype = {
        ...
        getSuggestionsLocal: function (query) {
            var that = this,
                ...
                data,
                isCached = that.cache.hasOwnProperty(query.substr(0, 3));

            // cache search terms on 3 letters typed
            if (query.length == 3 && !isCached) {
                that.cache[query] = $.grep(options.lookup, function (suggestion) { return filter(suggestion, query, queryLowerCase); });
            }

            data = {
                suggestions: $.grep((query.length >= 3 && isCached) ? that.cache[query.substr(0, 3)] : options.lookup, function (suggestion) {
