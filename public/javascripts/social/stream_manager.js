...
var StreamManager = Class.create({
  initialize: function() {
    ...
    this.mouse_is_in            = { convo_panel: false, list_item: false };
    this.clicked_ele            = { reply: true, rt: false, reply_focus: false, same_feed: null };
    this.twitterStreamLoadPref  = { initial_call: 0, show_old: 1, fetch_new: 2 };
    this.streamBoxElements      = {   list_item             : ".twt-list-item",
    ...
    this.socialPage             = { left_pane: ".tw-accs", center_pane:".center-pane", social_wrapper: ".tw-wrapper" }
  ...
  unifiedStreams: function(e){
    ...
    this.applyChanges();
    console.log("into unifiedStreams");

    var stream_ids = "", first_feed_id = "", last_feed_id = "";
  ...
  streamsItemInfo: function(e){
    ...
    params = { "stream_id" :     stream_id, "first_feed_id" : 0, "last_feed_id" :  0 }
  ...
  getLiveFeedData: function(search_query_hash, action) {
    ...
    j.ajax({
        ...
        success:  function(){
          j.event.trigger({ type: "streamLoadedEvent", message: "Success",time: new Date() }); //IMPORTANT TO REGISTER EVENTS AFTER STREAMS LOADED -  DOING IT with Custom Event
        }
    });
  },
  customSearchesItemInfo: function(e){
    ...
    j("#search_social #twitterquery").select2('val', search_hash['q']);
    j("#search_social #ignore-handles").select2('val', search_hash['exclude_handles']);
    j("#search_social #ignore-keywords").select2('val', search_hash['exclude_keywords']);
  ...
  recentSearchQL:function(e){
    ...
    j("#advsearch-box").slideDown();
    this.clearSearchInputs();

    query = j(e.currentTarget).attr('data-query').split(',');
    j("#search_social #twitterquery").select2('val', query);

    handle = j(e.currentTarget).attr('data-exclude-handles').split(',');
    j("#search_social #ignore-handles").select2('val', handle);

    keyword = j(e.currentTarget).attr('data-exclude-keywords').split(',');
    j("#search_social #ignore-keywords").select2('val', keyword);
  ...
  fetchStreamFeeds: function(action){
    ...
    params = {
      "stream_id"     : stream_ids, "first_feed_id" : first_ids, "last_feed_id"  : last_ids
    }
  ...
  invokeConversation: function(feed_id, reply_clicked, retweet_clicked){
    ...
    j.ajax({
        ...
        success : function(){
          j.event.trigger({ type: "convoLoadedEvent", message: "Success", time: new Date() });
        }
    });
  },
  /*Right Pane Actions */
  ...
  closeSocialConvoBox: function(){
    if(!j(this.convoElement).is(":hidden")){
      if(!j(this.eObjToTrackClick).hasClass("fsize-18")){//remove this line after TWIPSY removed
        console.log("Twipsy click check");
        if(!this.isTextAreaChanged) {
          console.log("called to close");
          this.closeConvo();
        }else{
          var msg = "You have unsaved changes. Are you sure you want to cancel the reply?";
          if (confirm(msg) == true) this.closeConvo();
          else return;
        }
      }
    }
