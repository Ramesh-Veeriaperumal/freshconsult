...
WorkflowMaxWidget.prototype = {
	WORKFLOW_MAX_FORM:new Template('<form id="workflow-max-timeentry-form"><div class="field first"><label>Staff</label><select name="staff-id" id="workflow-max-timeentry-staff" onchange="workflowMaxWidget.staffChanged(this.options[this.selectedIndex].value)" disabled class="full hide"></select> <div class="loading-fb" id="workflow-max-staff-spinner"></div></div><div class="field-35"><label>Job (Client)</label><select class="full hide" name="job-id" id="workflow-max-timeentry-jobs" onchange="workflowMaxWidget.jobChanged(this.options[this.selectedIndex].value)" disabled></select> <div class="loading-fb" id="workflow-max-jobs-spinner"></div></div><div class="field last"><label>Task</label><select class="full hide" disabled name="task-id" id="workflow-max-timeentry-tasks"></select> <div class="loading-fb" id="workflow-max-tasks-spinner" ></div></div><div class="field"><label id="workflow-max-timeentry-notes-label">Notes</label><textarea disabled name="notes" id="workflow-max-timeentry-notes" wrap="virtual">'+workflowMaxBundle.workflowMaxNote.escapeHTML()+'</textarea></div><div class="field"><label id="workflow-max-timeentry-hours-label">Hours</label><input type="text" disabled name="hours" id="workflow-max-timeentry-hours"></div><input type="submit" disabled id="workflow-max-timeentry-submit" value="Submit" onclick="workflowMaxWidget.logTimeEntry($(\'workflow-max-timeentry-form\'));return false;"></form>'),
	...
	handleLoadJob:function(staff_id) {
		...
		if(this.isRespSuccessful(this.jobData))
			selectedJobNode = UIUtil.constructDropDown(this.jobData, 'xml', "workflow-max-timeentry-jobs", "Job", "ID", ["Name", " ", "(", ["Client", "Name"], ")"], filterBy, searchTerm||"", false);
	...
	createTimeEntry:function(resultCallback) {
		if (workflowMaxWidget.validateInput()) {
			var body = this.CREATE_TIMEENTRY_REQ.evaluate({
				...
				hours: $("workflow-max-timeentry-hours").value*60,
