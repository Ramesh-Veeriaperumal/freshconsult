(function() {
    ...
    function createContainer() {
        if ($widget_attr.container == null) {
            ...
            $widget_attr.mobileArrowButton = document.getElementById('mobile-widget-arrow');
            ...
            bind($widget_attr.closeButton, 'click', function() {window.FreshWidget.close();Resize('hidesearch')});
            bind($widget_attr.mobileArrowButton, 'click', function() {MobileArrow('arrowclose')});
    ...
    function close() {
        $widget_attr.container.style.display = 'none';
        document.getElementById('freshwidget-dialog').classList.remove('freshwidget-dialog-box');
        document.getElementById('freshwidget-frame').classList.remove('iframeResize');
        if (!options.responsive) {
    ...
    function childFrameMessage() {
        bind(window, 'message', function(e) {
            ...
            if (message === 'showsearch' || message === 'hidesearch') {
                // show/hide search option in popup widget
                Resize(message);
            }
            if (message === 'get_parent_size') {
                // get parent size and send message to child
                ParentResize();
            }
            if (message === 'arrow') {
                // mobile arrow view
    ...
    bind(window, 'resize', ParentResize);

    // arrow hide/show for mobile popup widget
    function MobileArrow(message) {
        var doc = document;
        var closeElement = doc.getElementById('mobile-widget-close'),
            arrowElement = doc.getElementById('mobile-widget-arrow'),
            frame = doc.getElementById('freshwidget-frame');
        if (closeElement.offsetLeft > 0 && message === 'arrow') {
            closeElement.classList.add('mobile-widget-close-none');
            arrowElement.classList.add('mobile-widget-arrow-block');
        } else if (closeElement.offsetLeft <= 0 || message === 'arrowclose') {
            closeElement.classList.remove('mobile-widget-close-none');
            arrowElement.classList.remove('mobile-widget-arrow-block');
            if (message === 'arrowclose') {
                frame.contentWindow.postMessage('closewidget', '*');
            }
        }
    }

    // get parent size to add mobile responsive for iframe content
    function ParentResize() {
        var doc = document;
        var frame = doc.getElementById('freshwidget-frame'),
            parentWidth;
        if (doc.getElementById('FreshWidget'))
            parentWidth = doc.getElementById('FreshWidget').offsetWidth;
        if (frame && parentWidth) {
            if (parentWidth < 768) {
                doc.getElementById('freshwidget-dialog').classList.remove('freshwidget-dialog-box');
                doc.getElementById('freshwidget-frame').classList.remove('iframeResize');
            }
            frame.contentWindow.postMessage('offsetWidth:' + parentWidth, '*');
        }
    }

    //resize iframe for child request
    function Resize(data) {
        var doc = document;
        if (data == 'showsearch') {
            doc.getElementById('freshwidget-dialog').classList.add('freshwidget-dialog-box')
            doc.getElementById('freshwidget-frame').classList.add('iframeResize');
        } else if (data === 'hidesearch') {
            doc.getElementById('freshwidget-dialog').classList.add('freshwidget-dialog-box-close')
            doc.getElementById('freshwidget-frame').classList.add('iframeClose');
            doc.getElementById('freshwidget-dialog').classList.remove('freshwidget-dialog-box');
            doc.getElementById('freshwidget-frame').classList.remove('iframeResize');
            window.setTimeout(function() {
                doc.getElementById('freshwidget-dialog').classList.remove('freshwidget-dialog-box-close');
                doc.getElementById('freshwidget-frame').classList.remove('iframeClose');
            }, 800);
        }
    }
