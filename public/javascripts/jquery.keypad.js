...
function Keypad() {
	...
	this._noanimation = true; // Avoid any animations during show or hide
	this._keyCode = 0;
...
$.extend(Keypad.prototype, {
	...
	_showPlugin: function(field) {
		...
		inst._mainDiv.css({display: 'block', top: '-1000px', width: 'auto'});
		...
		// Temporary fix for keypad mis-alignment
		inst._input.after(plugin.mainDiv);
	...
	_backValue: function(inst) {
		...
		this._setValue(inst, (value.length == 0 ? '' :
			value.substr(0, range[0]) + value.substr(range[1])), range[0]);
		this.setCaretPosition(field, range[0]);
		this._notifyKeypress(inst, plugin.BS);
	},
	
	/* Set Caret Position inside the input field after 
	*/
	setCaretPosition: function (field, caretPos) {
		if (field.createTextRange) {
			...
			field.focus();
			if (field.selectionStart !== undefined) { field.setSelectionRange(caretPos, caretPos); }
		}
	},
	...
	_generateHTML: function(inst) {
		...
		for (var i = 0; i < layout.length; i++) {
			...
			for (var j = 0; j < keys.length; j++) {
				...
				if (keyDef) {
					...
					var additionalClass = " " + (this.keyClassMapping[keys[j]] || "");
					html += '<button type="button" class="' + this._keyClass + additionalClass +
						(inst.options.useThemeRoller ? ' ui-corner-all ui-state-default' : '') + '">' +
		...
		// used to provide transitions for onclick
		// optimize later
		var KEY_STATES = { mouseDownComplete: 0, mouseReleaseComplete: 1, timeOutComplete: 2 }
		var key_state;
		html.find('button').mousedown(function() {
			var _key = this;
			$(this).addClass(activeClasses);
			key_state = KEY_STATES['mouseDownComplete'];

			setTimeout(function() {
				key_state = _self.manageKeyStates( _key, key_state, 
																					KEY_STATES['mouseReleaseComplete'], 
																					KEY_STATES['timeOutComplete'], activeClasses );
			}, 200);

			if ($.isFunction(thisInst.options.onMousedown))
				thisInst.options.onMousedown.apply(this, [_self, thisInst]);
		}).
			mouseup(function() {
				key_state = _self.manageKeyStates( this, key_state, 
																					KEY_STATES['timeOutComplete'],
																					KEY_STATES['mouseReleaseComplete'], activeClasses );
			}).
			mouseout(function() { 
				key_state = _self.manageKeyStates( this, key_state, 
																					KEY_STATES['timeOutComplete'], 
	...
	manageKeyStates: function(key, key_state, toBeComparedState, toBeSetState, activeClasses) {
		if( key_state != toBeComparedState ) return toBeSetState;
		$( key ).removeClass( activeClasses );
		return key_state;
	},
