...
rules_filter = function(_name, filter_data, parentDom, options){
  ...
  var domUtil = {
    ...
    add_dom:
      function(ev_source){
        ...
        if (sys_events) {
        // only for observers
          // Loading seperate hash on performer on-change
          // "sys_events" contains system event dropdown hash

          var performerType = jQuery('input[name="performer_data[type]"]:checked').val();
          var isEventSourceAdder = jQuery(ev_source).hasClass('event_adder');

          if ((performerType === SYSTEM) && isEventSourceAdder ) {
            filterList = JSON.parse(sys_events);
          } else if ((performerType === CUSTOMER || ANYONE || AGENT) && isEventSourceAdder) {
            filterList = setting.orig_filter_data;
          }

          // Pushing new / original hash based on performer
          domUtil.add_to_hash(filterList);
        // end
        }

        $filter_select_list = FactoryUI.optgroup( getFilterInOptgroupFormat(filterList),
                                                  "name",
                                                  "ruCls_"+name+' select2',
    ...
    feed_data:
         ...
         function(dataFeed){
            ...
            if (sys_events) {
            // only for observers
              if (rule.name == "mail_del_failed_requester" || rule.name == "mail_del_failed_others") {
                filter_data = JSON.parse(sys_events);
  ...
  function init(){
      ...
      $doc.off('click.placedialog').on("click.placedialog", '.l_placeholder', function(ev){
        ev.preventDefault();
        ...
        .on("change", ".ruCls_"+name,
            function(){
              ...
              if(this.value !== -1){
                ...
                if (sys_events) {
                // only for observers
                  if (($this.val() == "send_email_to_agent") || ($this.val() == "responder_id")) {
                    if (jQuery('input[name="performer_data[type]"]:checked').val() === SYSTEM) {
                      // remove "event_performing_agent" option if System Event
                      $this.parent().children('div.dependent').children().find(("option[value='-2']")).remove();
                    }
                  }
                //end
                }
      ...
      jQuery(parentDom)
        ...
        if (sys_events) {
          var $ev_container   = jQuery('.event_container'),
              $ev_adder       = jQuery('.event_adder'),
              $all_container  = jQuery('.event_container, .condition_container, .action_container'),
              $all_adder      = jQuery('.event_adder, .condition_adder, .action_adder');
              var prev_performer = false; // true when previous-performer was SYSTEM
              // only for observers
              $doc.on("click.event_space", 'input[name="performer_data[type]"]', function() {
              var $current_performer = jQuery(this).val(),
              $removeButton   = jQuery('.rounded-minus-icon');
              var $actionSelector = jQuery('.ruCls_action.select2.select2-offscreen');
              if ($current_performer === SYSTEM) {
              $removeButton.trigger('click');
              $all_container.children().remove();
              $all_adder.trigger("click");
              $ev_adder.hide();
              jQuery('.event_container .sort_handle,.event_container .delete').addClass("disabled");
              $ev_container.children().addClass("single-field");
              prev_performer = true;
              } else if ($current_performer === CUSTOMER || ANYONE || AGENT) {
                if(prev_performer == true){
                  $removeButton.trigger('click');
                  $all_container.children().remove();
                  $all_adder.trigger("click");
                  $ev_adder.show();
                }
              prev_performer = false;
            }
          });

          $doc.on("ready.event_space", function() {
            jQuery('#performer_data_type_4').prev().addClass("separator-line");
            if (jQuery('input[name="performer_data[type]"]:checked').val() === SYSTEM) {
              jQuery('.event_container .sort_handle,.event_container .delete').addClass("disabled");
              $ev_container.children().addClass("single-field");
              $ev_adder.hide();
      ...
      $doc.on("click", ADD_DOM,
            function(){
              domUtil.add_dom(this);
            });
