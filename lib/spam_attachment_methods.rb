module SpamAttachmentMethods
	...
	def move_attachments(attachments, published_post)
		...
		(attachments['file_names'] || []).map do |file_name|
			object_presigned_url = AwsWrapper::S3.presigned_url(S3_CONFIG[:bucket], "#{folder}/#{file_name}", secure: true, expires_in: 1.hour.to_i)
			attachment = open(object_presigned_url) if object_presigned_url.present?
			if attachment
