...
module Reports::ChartGenerator
  ...
  def ticket_columns
    @ticket_columns ||= { "source" => TicketConstants.source_list,"priority" => TicketConstants.priority_list }
  end                        
  ...
  def gen_stacked_bar_data(value,column_name)
    ...
    pie_data = []
    sort_data.each do |key,tkt_hash|
     if (ticket_columns["source"].has_key?(key))
        pie_data.push({:name => ticket_columns["source"].fetch(key),  :data => [tkt_hash.to_f] })
     else
  ...
  def prepare_data_series(name,series_hash,options)
    ...
    unless series_hash.nil?
      series_hash.each do |tkt|
        series_data.push([DateTime.strptime(tkt.date.to_s, "%Y-%m-%d").to_time.to_i*1000, tkt.count.to_i])
        dates_with_data.push(Time.parse(tkt.date.to_s).to_i*1000)
      end
    end
