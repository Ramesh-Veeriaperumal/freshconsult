  ...
  private 
    ...
    def report_query
      %( #{select_columns}, count(freshfone_calls.id) as count,
          ...
          sum(if((freshfone_calls.ancestry is NOT NULL and freshfone_calls.call_status != #{Freshfone::Call::CALL_STATUS_HASH[:completed]}), 1,0)) as unanswered_transfers,
          ...
          ifnull(sum(if(freshfone_calls.call_status != #{Freshfone::Call::CALL_STATUS_HASH[:completed]} and freshfone_calls.call_type = #{Freshfone::Call::CALL_TYPE_HASH[:incoming]} and freshfone_calls.ancestry is NULL, 1, 0)), 0) as unanswered_call,
