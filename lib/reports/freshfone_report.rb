  ...
  private 
    ...
    def report_query
      %( #{select_columns}, count(freshfone_calls.id) as count,
          ...
          ifnull(sum(if(freshfone_calls.call_status != #{Freshfone::Call::CALL_STATUS_HASH[:completed]} and freshfone_calls.call_type = #{Freshfone::Call::CALL_TYPE_HASH[:incoming]}, 1, 0)), 0) as unanswered_call,
