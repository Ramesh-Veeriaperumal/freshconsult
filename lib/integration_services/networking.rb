  ...
  module Networking
    ...
    def http(options = {})
      @http ||= begin
        ...
        options[:ssl][:ca_file] ||= ca_file if self.class.default_http_options[:ssl][:verify].present?
    ...
    def faraday_builder(b)
      ...
      b.headers["User-Agent"] = @user_agent
    end
    ...
    def ca_file
      @ca_file ||= File.join(Rails.root,'config','cert','cacert.pem')
    end
    ...
    def verify_url(url_to_check)
      ...
      if (verified = @@verified_urls[uri.host]) == false
        raise IntegrationServices::Service::InvalidUrlError, "Invalid local address #{uri.host}"
      elsif verified
        return url_to_check
      end
