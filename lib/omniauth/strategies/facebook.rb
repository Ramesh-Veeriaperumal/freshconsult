    ...
    class Facebook < OmniAuth::Strategies::OAuth2
      ...
      def build_access_token
        with_authorization_code {         
          ...
          params = {'grant_type' => 'authorization_code', 'code' => verifier}.merge(client_params).merge({:redirect_uri => callback_url}.merge(options.token_params.to_hash(:symbolize_keys => true)))
          ...
          hash = (response.body.is_a? Hash) ? response.body : JSON.parse(response.body)
          raise Error.new(response) if client.options[:raise_errors] && !(response.parsed.is_a?(Hash) && hash['access_token'])
