...
module APIHelperMethods
  ...
  def convert_query_to_conditions(query_str)
    ...
    if ALLOWED_QUERY_COLUMNS.include?(conditions[0]) && conditions[1].match(/(like|is)/)
      ...
      query[0] = conditions[0] == 'customer_id' && Account.current.multiple_user_companies_enabled? ?
                                    "user_companies.company_id #{OPERATORS[conditions[1]]} ?" :
                                    "#{conditions[0]} #{OPERATORS[conditions[1]]} ?"
