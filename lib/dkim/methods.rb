module Dkim::Methods
  ...
  def handle_dns_action(action, record_type, record_name, record_value)
    ...
    $route53_pod_info.change_resource_record_sets(hosted_zone_id: DNS_CONFIG['hosted_zone'], change_batch: { changes: [build_record_attributes(action, record_type, record_name, record_value)] })
  end

  def new_record?(domainkey, record_type)
    domainkey = eval(domainkey)
    response = $route53.list_resource_record_sets(hosted_zone_id: PodDnsUpdate::DNS_CONFIG['hosted_zone'], start_record_name: domainkey, start_record_type: record_type, max_items: 1)
