	...
	module Scoreboard
		class ProcessTicketScore < Resque::FreshdeskBase
			...
			def self.perform(args)
				...
				args[:remove_score] ? remove_score(ticket) : add_score(ticket, args[:fcr])
			end

			def self.add_score(ticket, first_call_resolution)
				SupportScore.add_support_score(ticket, ScoreboardRating.resolution_speed(ticket))
    		SupportScore.add_fcr_bonus_score(ticket) if first_call_resolution
			end
