module Social::Twitter::Util
  ...
  def add_as_note(twt,twt_handle, twt_type, ticket, options={})
    ...
    begin
      ...
      if note.save_note
        ...
        puts "error while saving the ticket - #{note_hash[:tweet_id]} :: #{note.errors.to_json}"
      end
    ensure
  ...
  def construct_params(twt, options)
    hash = {
      :body => options[:gnip] ? twt[:body] : twt.text,
      :tweet_id => options[:gnip] ? twt[:id].split(":").last.to_i : twt.id ,
      :posted_time => options[:gnip] ? Time.at(Time.parse(twt[:postedTime]).to_i) : Time.at(twt.created_at).utc,
      :sender => options[:gnip] ? @sender : @sender.screen_name
    }
  end
end
