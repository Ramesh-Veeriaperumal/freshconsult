  ...
  protected
    def self.load_conditions(user)
      ...
      
      {
        ...
        :open_or_pending  => ["status in (?, ?) and deleted=?" , STATUS_KEYS_BY_TOKEN[:open], STATUS_KEYS_BY_TOKEN[:pending] , false],
        :resolved_or_closed  => ["status in (?, ?) and deleted=?" , STATUS_KEYS_BY_TOKEN[:resolved], STATUS_KEYS_BY_TOKEN[:closed],false]
      }
    end
