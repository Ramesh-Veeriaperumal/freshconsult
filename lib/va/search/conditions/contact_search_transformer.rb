class VA::Search::Conditions::ContactSearchTransformer
  ...
  def fetch_custom_field(name)
    contact_form_fields.find { |t| t.name == name.to_s }
  end

  def custom_dropdown_values(custom_field, values)
    choices = []
    choices.push(DEFAULT_ANY_NONE[ANY_NONE[:NONE]]) if values.include?(ANY_NONE[:NONE])
    custom_field.choices.each do |choice|
