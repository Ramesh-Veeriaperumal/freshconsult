  ...
  class EmailServiceEmailVerifier
    ...
    include ParserUtil
      ...
      def validate_request(account_id, account_name, account_domain)
        ...
        headers = { "Authorization"  => EmailServiceConfig::API_KEY, "Content-Type" => "application/json" }
        parsed_response = {}
        begin
          Timeout.timeout(60) do 
            response = HTTParty.send('post', "#{EmailServiceConfig::URL}#{EmailServiceConfig::ACCOUNT_VALIDATE_URLPATH}", :body => @parameters,:headers => headers)
