...
module Helpdesk::S3::ArchiveNote
  module Body
    ...
    module ClassMethods  
      ...
      def generate_file_path(account_id,ticket_id)
        generate_key(account_id,ticket_id) + "/archive_note_body.json"
      end

      # gets the file from s3 based on account_id and ticket_id
      def get_from_s3(account_id,ticket_id)
        read(generate_file_path(account_id,ticket_id),S3_CONFIG[:archive_note_body])
      end
    end
