module Helpdesk::Email::NoteMethods
  ...
  def rsvp_to_fwd?
    @rsvp_to_fwd ||= ((Account.current.features?(:threading_without_user_check) || ticket.cc_email[:cc_emails].include?(email[:from][:email]) || user.agent?) && reply_to_forward(header_processor.all_message_ids))
  end
