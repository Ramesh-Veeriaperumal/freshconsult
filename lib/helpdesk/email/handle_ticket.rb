...
class Helpdesk::Email::HandleTicket
  ...
  def remove_unwanted_email_ids
    ...
    self.email[:cc] = email[:cc].reject{ |cc_hash| kbase_email?(cc_hash[:email]) or requester_email?(cc_hash[:email]) }
    self.email[:to_emails] = email[:to_emails].reject{ |cc_hash| kbase_email?(cc_hash[:email]) or requester_email?(cc_hash[:email]) }
  end
