class ControllerLogSubscriber <  ActiveSupport::LogSubscriber
  ...
  def logging_format(payload)
    ...
    log_file_format = "server_name=#{payload[:server_name]}, uuid=#{payload[:uuid]}, ip=#{payload[:ip]}, a=#{payload[:account_id]}, u=#{payload[:user_id]}, s=#{payload[:shard_name]}, d=#{payload[:domain]}, url=#{payload[:url]}, path=#{payload[:path]}, c=#{payload[:controller]}, action=#{payload[:action]}, host=#{payload[:server_ip]}, status=#{payload[:status]}, format=#{payload[:format]}, db=#{payload[:db_runtime]}, view=#{payload[:view_runtime]}, duration=#{payload[:duration]}"
  end
