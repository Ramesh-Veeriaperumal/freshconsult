module Helpdesk::AdjacentArchiveTickets
	...
	private
		...
		def find_in_adjacent_pages(direction)
			...
			if filter_params
				...
				filter_params[:select_fields] = "archive_tickets.id, archive_tickets.display_id"

				tickets = current_account.archive_tickets.permissible(current_user).filter(
					:params => filter_params, 
					:filter => 'Helpdesk::Filters::ArchiveTicketFilter') unless filter_params[:page] <= 0
		...
		def criteria
			@adjacent_tickets_criteria ||= begin
				...
				# 	@ticket_filter.query_hash = filter_params[:data_hash] unless filter_params[:data_hash].blank?
		...
		def adjacent_meta_key
			prepare_redis_key(HELPDESK_ARCHIVE_TICKET_ADJACENTS_META)
		end

		def adjacent_list_redis_key
			prepare_redis_key(HELPDESK_ARCHIVE_TICKET_ADJACENTS)
		end

		def cached_filters_key
			prepare_redis_key(HELPDESK_ARCHIVE_TICKET_FILTERS)
		end
