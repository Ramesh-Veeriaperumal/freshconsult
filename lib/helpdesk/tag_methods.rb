module Helpdesk::TagMethods
  def update_tags(tag_list, remove_tags, item)
    ...
    remove_ticket_tags(old_tag_list.select { |tag| !new_tag_list.any? { |new_tag| new_tag.casecmp(tag).zero? } }, item) unless !remove_tags
    update_ticket_in_es(item) if tag_list.present?
  end
  ...
  def update_ticket_in_es(ticket)
    if Account.current.launched?(:es_count_writes)
      SearchSidekiq::TicketActions::DocumentAdd.perform_async(klass_name: 'Helpdesk::Ticket', id: ticket.id, version_value: Search::Job.es_version)
    end
    Search::Dashboard::Count.new(klass_name: 'Helpdesk::Ticket', document_id: ticket.id, version: Search::Job.es_version).index_es_count_document
  end
  ...
  def restore_dirty_tags(item)
    if item.is_a?(Helpdesk::Ticket)
      unless item.deleted? || item.spam?
        item.dirty_attributes[:tag_attributes].each do |key, value|
          tag = Account.current.tags.find_by_id(key)
          item.tags << tag ? tag : Account.current.tags.new(name: value, tag_uses_count: 1)
        end
        item.dirty_attributes[:tag_attributes] = {}
      end
    end
