module UsageMetrics::Features
  ...
  FEATURES_TRUE_BY_DEFAULT = [:default_ticket_view, :private_note, 
    ...
    :helpdesk_in_depth, :timesheet_reports, :scheduling_reports, :customer_analysis,
    :drill_down_to_tickets, :ticket_volume_trends, :performance_distribution].freeze
  ...
  def metrics(account, shard, features_list)
    Hash.new.tap do |feature_usage|
      ...
      features_list.each do |feature|
        begin
          feature_usage[feature] = self.safe_send(feature, args)
        rescue => e
