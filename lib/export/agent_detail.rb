module Export
  class AgentDetail
    ...
    def initialize(params={})
      @args = params
      @csv_hash = params[:csv_hash]
      @portal_url = params[:portal_url]
      @send_mail = (params[:send_mail].nil? ? true : params[:send_mail]) # true by default(storing in s3 and send email to user)
    end

    def perform
      @agents = Account.current.agents.includes(preload_associations)
    ...
    private
      ...
      def skills_name agent
        agent.user.skills.map(&:name).join(" , ")
      end
  end
end
