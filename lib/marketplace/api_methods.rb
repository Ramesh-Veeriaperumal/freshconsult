module Marketplace::ApiMethods
  ...
  private
    ...
    def list_api_url(account_id)
      format(Marketplace::ApiEndpoint::ENDPOINT_URL[:installed_extensions],
             product_id: Marketplace::Constants::PRODUCT_ID,
             account_id: account_id)
    end

    def show_api_url(extension_id)
      format(Marketplace::ApiEndpoint::ENDPOINT_URL[:extension_details],
             product_id: Marketplace::Constants::PRODUCT_ID,
             extension_id: extension_id)
    end

    def fetch_installed_extensions(acc_id, types)
      api_url = list_api_url(acc_id)
    ...
    def log_on_error(response)
      ...
      exception_logger("Error in fetching installed app caller method: \
        #{caller(1..1).first}, error: #{response.inspect}")
    end
end
