  ...
  module Migration
    ...
    def save_freshcaller_account(response)
      ...
      Freshcaller::Account.create(
        ...
        freshcaller_account_id: response[:freshcaller_account_id],
    ...
    def upload_to_s3(freshcaller_account_id)
      Dir.foreach(account_migration_location) do |file|
        ...
        AwsWrapper::S3Object.store(S3_LOCATION + "/#{freshcaller_account_id}/#{file}", data, FreshcallerConfig['s3_bucket'], content_type: 'json')
      end
    ...
    def initiate_migration(account_detail)
      ...
      freshcaller_request({ helpkit_account: current_account.id,
                            ...
                            account_id: account_detail[:freshcaller_account_id] },
                            "#{protocol}#{FreshcallerConfig['domain_prefix']}#{account_detail[:freshcaller_account_domain]}/migrate",
                            :post)
    end
  end
end
