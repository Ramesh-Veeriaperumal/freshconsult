class Workers::FbSplitTickets
  extend Resque::AroundPerform 
  @queue = 'fb_split_tickets'
  
  def self.perform(args)
    @account = Account.current
    user = @account.users.find(args[:current_user_id])
    user.make_current
  ...
  def self.update_ticket_activity
  ...
  def self.set_notable_objects(source_ticket_id, comment_ticket_id, child_post_ids)
