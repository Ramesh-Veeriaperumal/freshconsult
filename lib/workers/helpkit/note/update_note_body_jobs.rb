class Workers::Helpkit::Note::UpdateNoteBodyJobs
  extend Resque::AroundPerform
  @queue = "helpdesk_update_note_body_queue"
  WorkerUpdateNoteBodyDelay = 1
  WorkerUpdateNoteBodyRetry = 4

  class << self
    def perform(args)
      begin
        bucket = S3_CONFIG[:note_body]
        note_body = Helpdesk::NoteOldBody.find_by_note_id_and_account_id(args[:key_id],args[:account_id])
        args[:data] = note_body.attributes
        Helpdesk::S3::Note::Body.push_to_s3(args,bucket)
      rescue Exception => e
        # need to put the back to the resque
        args[:retry] = args[:retry].to_i + 1
        if args[:retry] < WorkerUpdateNoteBodyRetry
          args.delete(:data)
          Resque.enqueue_at(WorkerUpdateNoteBodyDelay.minutes.from_now, self, args)
        else
          raise e
        end
      end
    end
  end
end
