  ...
  task :ticket_load => :environment do
    ...
    shards.each do |shard_name|
     ...
     Sharding.run_on_shard(shard_name.to_sym) {
      check_for_spam('helpdesk_tickets', 'requester_id', get_redis_values("#{shard_sym}:tickets_limit"), SPAM_TICKETS_THRESHOLD,shard_name)
      check_for_spam('helpdesk_notes', 'user_id',get_redis_values("#{shard_sym}:notes_limit") , SPAM_CONVERSATIONS_THRESHOLD,shard_name)
     }
    end
    ...
    unless user_ids.empty?
      ...
      account_ids.keys.each do |account_id|
        ...
        unless deleted_users.empty?
          ...
          SubscriptionNotifier.send_email(:deliver_admin_spam_watcher, account.admin_email, account, deleted_users)
        end
      end
    end
end
