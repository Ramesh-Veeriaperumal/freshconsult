...
def check_for_tickets_spam(table,column_name, threshold)
  ...
  query_str = <<-eos
    ...
    between '#{60.minutes.ago(current_time).to_s(:db)}' and '#{current_time.to_s(:db)}'  and id > 7000000
...
def check_for_notes_spam(table,column_name, threshold)
  ...
  query_str = <<-eos
    select #{column_name},count(*) as total from #{table} where created_at 
    between '#{60.minutes.ago(current_time).to_s(:db)}' and '#{current_time.to_s(:db)}' and id > 6000000
    group by #{column_name} having total > #{threshold}
  eos
  puts query_str
  requesters =[]
