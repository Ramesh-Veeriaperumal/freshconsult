namespace :seed_tickets do
  ...
  def create_fb_ticket(options = {})
    ...
    fb_ticket = Account.current.tickets.build(
      ...
      created_at: Time.now, #Time.zone.parse(options[:post][:created_time]),
  ...
  def fb_post_params(options = {})
    {
      ...
      msg_type:     ["post", "dm"].sample, #msg type can either be a direct message (dm) or a post
  ...
  def get_fb_post_attributes(options = {})
    ...
    msg_type = options[:post][:msg_type] || ["post", "dm"].sample 
