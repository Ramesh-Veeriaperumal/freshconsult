class Solution::Builder
	...
	class << self
		...
		def build_obj(obj)
			...
			@solution_obj.language_id = args[obj][:language_id] || Language.default
		end

		def assign_meta_associations(obj)
			return unless META_ASSOCIATIONS.keys.include?(obj)
			@meta_obj.send("#{META_ASSOCIATIONS[obj]}_meta=", get_parent_association(obj))
		end

		def assign_meta_attributes(obj)
		...
		def get_parent_association(obj)
