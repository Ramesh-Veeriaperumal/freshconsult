module HelpdeskReports::Helper::QnaInsightsReports
  ...
  def get_insights_widget_config (widget_id=nil)
    ...
    final_config.delete(:d10) if (Account.current.hide_agent_metrics_feature? || scope == :assigned_tickets) 
    final_config.delete(:d11) if (User.current.agent.agent_groups.count < 2 && ( scope == :group_tickets || scope == :assigned_tickets )) 
    final_config
  end
