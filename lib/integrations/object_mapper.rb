class Integrations::ObjectMapper
    ...
    def map(data_hash, map_configs, convertion_type)
      ...
      map_configs.each {|map_config|
        ...
        if convertion_config.blank?
          ...
          if data_hash[:input].cloud_files.length > 0 && convertion_type == :ours_to_theirs  &&  convertion_config[:handler].nil? && data_hash[:input].status.nil?
        ...
        :map=>[
          {:ours=>"note_body_content", :theirs_to_ours=>{:handler=>:db_fetch, :create_if_empty=>true, :entity=>Helpdesk::NoteBody,
                          :create_params => {:body => "JIRA comment {{notification_cause}} # {{comment.id}}:\n {{comment.body}}\n",
                                             :body => "JIRA comment {{notification_cause}} # {{comment.id}}:\n {{comment.body}}\n", 
  ...
  PRIVATE_NOTE_CONFIG[:map].push({:ours=>"to_emails",:theirs_to_ours=> {:handler=>:db_fetch, :entity=>User, :data_type => "String",:field_type => "email", 
                         :using=>{:select=>"users.email,users.helpdesk_agent",
