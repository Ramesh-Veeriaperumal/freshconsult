module RabbitMq::Subscribers::TicketBodies::Activities
  ...
  def mq_activities_ticket_body_properties(action)
    self.ticket.to_rmq_json(ACTIVITIES_TICKET_KEYS, action)
  end

  def ticket_body_subscriber_properties(action)
  ...
  def mq_activities_ticket_body_valid(action, model)
    valid_ticket_body_model?(model) && ticket_body_valid?
  end

  private

  def valid_ticket_body_model?(model)
    model == "ticket_body"
  end

  def ticket_body_valid?
