module RabbitMq::Subscribers::Search::SqsUtils
  ...
  def manual_publish(model_object)
    ...
    model_message   = RabbitMq::SqsMessage.skeleton_message(model_name, action, model_uuid, model_object.account_id)
    ...
    RabbitMq::Utils.manual_publish_to_xchg(
      model_uuid, model_exchange, model_message.to_json, RabbitMq::Constants.const_get("RMQ_SEARCH_#{model_name.upcase}_KEY")
    )
  end
