...
class RemoteFile < ::Tempfile
  ...
  def fetch
    string_io = OpenURI.send(:open, @remote_path, :http_basic_authentication => [username , password]) # calling send directly because open is a private method
  ...
  def fetch_without_authentication
    string_io = OpenURI.send(:open, @remote_path) # calling send directly because open is a private method
    self.open_uri_path = string_io.path if string_io.respond_to?(:path)
    self.write string_io.read.force_encoding("UTF-8")
    self.rewind
    self
