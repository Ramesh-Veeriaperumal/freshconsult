module Dashboard::ElasticSearchMethods
  ...
  def fetch_unresolved_tickets_from_es(unresolved = false)
    ...
    es_response = Search::Dashboard::Docs.new(action_hash,negation_conditions,[@group_by,"status"]).aggregation(Helpdesk::Ticket)["name"]["buckets"]
    ...
    missing_es_response = Search::Dashboard::Docs.new(action_hash,negation_conditions,["status"]).aggregation(Helpdesk::Ticket)["name"]["buckets"]
  ...
  def widget_count_from_es(group_by, unresolved = false, my_view = false)
    ...
    es_response = Search::Dashboard::Docs.new(action_hash,negation_conditions,[group_by]).aggregation(Helpdesk::Ticket)["name"]["buckets"]
