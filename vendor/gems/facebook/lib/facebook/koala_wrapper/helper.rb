module Facebook
  module KoalaWrapper
    module Helper
       ...
       def post_hash(post_id, type, user_name, user_id, message, created_at, share_count = 0)
        {
          :id => post_id,
          :type => type,
          :from => {
      ...
      def post_from_dynamo(fb_feed, type)   
        ...
        post_hash(post_data[:feed_id], type, post_data[:requester][:name], post_data[:requester][:id], post_data[:description], post_data[:created_at]) 
      end
      
      def comment_from_dynamo(fb_feed, type, can_comment, parent = nil)   
        comment_data = JSON.parse(fb_feed["data"][:ss][0]).deep_symbolize_keys
        comment_hash(comment_data[:feed_id], type, comment_data[:requester][:name], comment_data[:requester][:id], comment_data[:description], parent, comment_data[:created_at], can_comment) 
      end
