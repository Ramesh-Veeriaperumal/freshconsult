class Social::FacebookPage < ActiveRecord::Base
  ...
  scope :active, -> { where(enable_page: true) }
  scope :reauth_required, -> { where(reauth_required: true) }
  scope :realtime_messaging_disabled, -> { where(realtime_messaging: false) }
  scope :valid_pages, -> { where(reauth_required: false, enable_page: true ) }
