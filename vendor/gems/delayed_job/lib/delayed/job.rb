...
module Delayed
  ...
  class Job < ActiveRecord::Base
    ...
    def reschedule_without_lock(message, account_id, backtrace = [], time = nil)
      if self.attempts < MAX_ATTEMPTS
        reschedule_timespan = (attempts ** 4) + 5
        reschedule_timespan = 4.hours if (reschedule_timespan > 4.hours) 
        time ||= Job.db_time_now + reschedule_timespan
    ...
    def run_without_lock (account_id)
      ...
      begin
        ...
        reschedule_without_lock e.message, account_id, e.backtrace
    ...
    def self.enqueue(*args, &block)
      ...
      if smtp_mailboxes.any?{|smtp_mailbox| smtp_mailbox.enabled?}
        ...
        Object.const_get("DelayedJobs::MailboxJob").perform_async({:job_id => job.id, :account_id => account_id}) if job && job.id && PUSH_QUEUE.include?(job_queue)
        job
      else
