class PodShardCondition < ActiveRecord::Base
  ...
  def self.fetch_details
    ...
    key = MemcacheKeys::POD_SHARD_ACCOUNT_MAPPING % { :pod_info => PodConfig['CURRENT_POD'], :shard_name => current_shard }
  ...
  private
    def clear_cache
      unless shard_name_was.blank?
        key = MemcacheKeys::POD_SHARD_ACCOUNT_MAPPING % { :pod_info => PodConfig['CURRENT_POD'], :shard_name => shard_name_was }
