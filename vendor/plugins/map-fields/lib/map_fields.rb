  ...
  def map_fields
    ...
    if session[:map_fields].nil? || !params[options[:file_field]].blank?
      ...
      if session[:map_fields][:file].nil? || params[:fields].nil?
        ...
        #@mapped_fields = MappedFields.new(session[:map_fields][:file],expected_fields, params[:fields],params[:ignore_first_row])
        @mapped_fields = FasterCSV.read(session[:map_fields][:file], :headers => :first_row)
      end
    end
