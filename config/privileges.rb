...
Authority::Authorization::PrivilegeList.build do
  ...
  admin_tasks do
    ...
    resource :account, only: [:update, :edit, :delete_logo, :delete_favicon, :manage_languages, :update_languages, :enable_skip_mandatory, :disable_skip_mandatory]
    ...
    resource :api_contact_field, :only => [:index]
    resource :api_company_field , :only => [:index]
    resource :"admin/ticket_field", only: [:create]
